...
{
        if( query_temp("item") ) return notify_fail("幹將正忙着呢，稍等會。\n");
        ...
        message_vision("幹將冷冷的對$N說道：你給我這些東西干什"
                       "么？我煉劍，不用這些東西。\n", me);
        return 0;
}
...
int do_finish()
{
        ...
        if (! objectp(ob = query_temp("item/making")))
        {
                // 數據不對，可能是由於BUG造成，也可能是因為幹將被update過
...
// 生成道具文檔, 需要一個的道具原料對象
// 生成以後建立 ITEM_DIR/???.c 然後刪除舊的原料對象, 生成一件新的道具，返迴文件名
...
int do_notch(string arg)
{
        ...
        if( query("balance", me)<10000000 )
                return notify_fail("你在錢莊里的存款不夠支付道具開槽費用。\n");
...
int do_suit(string arg)
{
        ...
        */
        ...
        if (query("balance", me) < 10000000)
                return notify_fail("你銀行里的錢不夠吧，為" + obj->name() + NOR "綠化套裝需要" +
...
int do_upsuit(string arg)
{
        ...
        */
        ...
        if (query("balance", me) < 100000000)
                return notify_fail("你銀行里的錢不夠吧，為" + ob->name() + NOR "提升套裝品級需要" +
...
int do_increase(string arg)
{
        ...
        */
        ...
        // 需要黃金一萬兩
        if( query("balance", me) < 100000000 )
                return notify_fail("你的銀行里好象沒有備齊1萬兩黃金吧。\n");
...
int do_signature(string arg)
{
        ...
        if( query("balance", me) < 10000000 )
                return notify_fail("你在錢莊里的存款不夠支付裝備簽名費用。\n");
...
int do_cuilian(string arg)
{
        ...
        if( query("balance", me) < 10000000 )
                return notify_fail("你銀行里的錢不夠吧，為" + item->name() + NOR "淬鍊需要" +
                                   HIY "一千兩黃金，" NOR "還是湊夠再來吧！\n");

        addn("balance", -10000000, me);

        message_vision(HIM "$N接過$n" HIM "手中的" + item->name() + HIM "，仔細端詳了半天，嘆道：明珠蒙塵，可惜啊可惜！！！\n"
                     "也罷，既然你找到好的材料，那我就幫你淬鍊一番。說完，隨手將" + item->name()+HIM"和"+ob->name() + HIM "拋入爐\n" +
                     "內，霎時間，那" + item->name()+HIM"和"+ob->name() + HIM "已被火爐中的熊" HIR "烈焰" HIM "吞沒。\n"
                     "$N凝視着爐火，仰天嘆道：天地之精華為其血脈，日月之光輝是其靈氣......\n" NOR,
                     this_object(), me);
