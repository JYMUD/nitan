...
void create()
{
        set_name("游訊", ({ "you xun", "you", "xun", "youxun" }) );
        set("nickname", HIG "滑不留手" NOR);
        set("gender", "男性" );
        set("age", 42);
        set("long", "他就是專門打探、販賣消息的游訊。\n");
        ...
        set("chat_chance", 1);
        set("chat_msg",({
                "游訊嘻嘻道：小的我沒別的本事，就會打聽消息，就算是巫師造謠，我也能知道。\n",
                "游訊亮出一指道：便宜啊，幾兩銀子就能查條謠言，這可真是跳樓價。\n",
                "游訊賊眯眯的說：天算地算不如人算，可是再怎麼算我游某都知道。\n",
        }));
...
mixed accept_ask(object ob, string topic)
{
        ...
        if (topic[0] > 160)
        {
                ...
                {
                        tell_object(ob, HIY "游訊朝你一瞪眼睛：『別吵！問問題是吧？問那麼急幹嘛？』\n" NOR);
                        return 1;
                }
                ...
                if (sizeof(obs) > 0)
                {
                        ...
                        message_vision(CYN "$N" CYN "嘿嘿奸笑兩聲，對$n"
                                ...
                                "金，不二價！』\n" NOR, this_object(), ob);
                        */
                        tell_object(ob, CYN "游訊嘿嘿奸笑兩聲，對你"
                                        CYN "小聲道：『沒有問題，不過得要一兩黃"
                                        "金，不二價！』\n" NOR);
                        return 1;
                } else
...
int recognize_apprentice(object me, string skill)
{
        if (skill != "idle-force")
        {
                if( me->add_temp("illegal_learn",1)>3 )
                {
                        ...
                        command("say 我只教發獃神功，不傳其它武功！");
                return -1;
        }

        message_vision("$N向$n請教發獃神功的訣竅。\n", me, this_object());
...
int accept_object(object me, object ob)
{
        ...
        if( stringp(wid=query_temp("pending/ask_youxun", me)) )
        {
                ...
                if (wid[0] > 160)
                {
                        ...
                        tell_object(me, WHT"游訊在你的耳邊悄聲說道：" + msg + "\n" NOR); 
                        return 1;
                }
                ...
                */
                tell_object(me, WHT "游訊在你的耳邊悄聲說道：據可靠消息，這個人剛才在" +
                        environment(fob)->short() + "。\n" NOR);
                return 1;
        }
...
void receive_report(object user, string verb)
{
        ...
        message("vision", HIG "游訊悄悄的告訴你：" + msg + "\n",
                receiver, user);
}
