...
int do_qiuqian()
{
        ...
        if (present("zhu qian",me))
                return notify_fail("你身上不是有一根簽么？好好琢磨琢磨吧。\n");
        ...
        message_vision("$N趕緊放下籤筒，俯身把簽揀了起來，小心翼翼的收入懷中。\n",me);
...
int do_answer(string arg)
{
        ...
        switch (arg)
        {
                ...
                default:
                        command("yi");
                        command("consider");
                        message_vision("$N盯着簽尋思良久，臉色愈發凝重，突然將簽塞回$n手中，卻是一言不發！\n",this_object(),me);
                        obj->move(me);
                        return 1;
                        break;
        }
...
int accept_object(object me, object ob)
{
        ...
        if( query("money_id", ob) )
        {
                ...
                {
                        command("say 沒事給老夫錢幹嘛？當老夫是叫化子么？");
                        return 0;
                }
                if( ob->value()<query_temp("have_gived", me) )
                {
                        command("say 就這點錢也來糊弄老夫？當老夫窮瘋了么？");
                        return 0;
                }
