...
void create()
{
        ...
        if (clonep())
                ...
                set("long", sort_string(HIW "一個四周邊緣環繞着神秘光環的箱子，據說"
...
string extra_long()
{
        ...
        } else
        {
                if (query("closed"))
                        ...
                        "開（open）箱子才可以查看（look）箱子里的物品。" NOR;
                else
...
int do_open(string arg)
{
        ...
        set("closed", 0);
        save();
        write(HIG "箱子已經成功打開，可輸入 look 查看箱子里裝的東西！\n" NOR);
        return 1;
}
...
int get_passwd(string pass, object ob)
{
        if (crypt(pass, query("passwd")) == query("passwd"))
        {
                ...
                set("closed", 0);
                save();
                write(HIG "箱子已經成功解鎖，可輸入 look 查看箱子里裝的東西！\n" NOR);
                return 1;
        } else
        {
                write("密碼錯誤，請核對密碼後再試！\n");
...
int do_take(string arg)
{
        ...
        if (! store || sizeof(store) < 1 || sn > sizeof(store))
                return notify_fail("你的箱子里沒有存放這項物品。\n");
        ...
        if (ob->query_amount())
        {
                ...
                ob->move(me, 1);
                save();
                message_vision("$N從箱子里取出一"+query("unit", ob)+query("name", ob)+"。\n",me);
                return 1;
        }
        ...
        save();
        if( ob )
        message_vision("$N從箱子里取出" + chinese_number(amount) +
