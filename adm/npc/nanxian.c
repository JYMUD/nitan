...
void create()
{
        ...
        set("long", @LONG
此人衣着青杉，鬚髮皆白。看似其貌不揚，實
卻為世上頭號人物。這便是武林中號稱「南賢
北丑」的南賢。其為人亦正亦邪，年輕時曾干
...
*/
                if( query("quest_tuteng/end", me) )
                        return "不錯，不錯，閣下了卻了南賢的心愿，多謝！\n";
                if( query("quest_tuteng/start", me) )
                        return "你已經領取了該任務，請儘快搜集七張帝王符圖交給我吧！\n";
        ...
        if (topic == "修羅道")
        {
                ...
                if( query("int", me)<32 || 
                    ...
                    query("str", me)<32 || 
                    query("dex", me)<32 )
                    return "你的先天屬性還不足以修鍊修羅道，我看你還是先回去吧。\n";
                ...
                if( query("lunhui-sword_quest/xiuluodao/finish", me) && 
                    me->query_skill("xiuluodao", 1))
                        return "呵呵，多虧你上次幫我，這無敵劍氣我已經修鍊成功了。\n";
        
                // 分配任務
                if( !query("lunhui-sword_quest/xiuluodao/give_quest", me) )
                {
                        command("hehe");
                        command("say 我正在修鍊渾天寶鑒之無敵劍氣，急需1把青罡劍練功！");
                        command("ke");
                        command("say 能幫我這個忙嗎？");
                ...
                command("hehe");
                command("nod");
                command("say 既然你幫我了這個忙，我就順便傳你一招，相信你以後用得着 ...");
...
/*
        message("member", HIM "\n【泥潭會員】" + me->query_idname() + HIM " 領取二小時的高效 飯κ奔淇技剖薄n" NOR,
                all_interactive(), 0);
*/
...
int full_all()
{
        ...
        ob->start_busy(2);
        message_vision(CYN "$N微笑着伸手朝$n一拂道聲：“好了！”\n" NOR,me, ob);
...
mixed ask_buchang()
{
        ...
        me->save();
        tell_object(me, HIY "\n你領取了(" HIM "當機補償" NOR + HIY ")，祝你在王者之戰里玩的愉快！\n" NOR);
        return 1;
}
...
mixed ask_reborn()
{
        ...
        {
                message_vision("$N對$n一拱手，道： 鬩丫纜只亓耍銥淳筒揮昧稅桑n",
                               this_object(), who);
                return 1;
        }
        ...
        {
                message_vision("$N對$n一拱手，道：你還沒有修鍊元嬰出世呢。\n",
                               this_object(), who);
                return 1;
        }
