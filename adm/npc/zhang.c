...
int do_show(string arg)
{
        ...
        {
                command("say 你亮出錢幹什麼？想獻給觀里？那就給我好了。");
                return 1;
        }
        ...
        message("vision", sort_string(HIM + name() + "從口袋中摸出一張黃色"
                                ...
                                "“太上老君急急如律賜令！”說罷一抖掌中劍，"
                                "喝道：“疾”！\n" NOR, 64), environment());
        message_vision("焰光划出一道光華，$N發了發獃，也不知道算是怎麼回事。\n",
                       me);
...
string ask_tianjidao()
{
        ...
        if( query("int", me)<32 || 
            ...
            query("str", me)<32 || 
            query("dex", me)<32 )
            return "你的先天屬性還不足以修鍊天極道，我看你還是先回去吧。\n";
        ...
        if( !query("lunhui-sword_quest/tianjidao/give_quest", me) )
        {
                ...
                command("say 如果能搜集到30根天蠶絲，老夫倒是有辦法重新製作一。");
                command("不過……");
                command("tan");
