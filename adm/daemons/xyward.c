...
// 襄陽保衛戰序列號 RAN_NUM
// 該序列號在每次開始前隨機產生，作用是標註本次襄陽保衛戰鬥的唯一性
...
{
        status = THEEND;
        xywar_message(HIR "提示：本次襄陽保衛戰由於報名人數不足，宣告結束。\n" NOR);
...
public void war_win()
{
        ...
        call_out("xywar_message", 4, HIW "由於襄陽守軍及玩家地奮力抵抗，終於擊退了蒙古軍，襄陽保衛戰勝利！\n");
...
void war_end_player()
{
        ...
        xywar_message(HIR "由於參與襄陽保衛戰的玩家已剩餘不到五名，難以繼續抵抗蒙古人的攻勢。" NOR);
...
void set_now_status(int flag)
{
        ...
        {
                status = THEEND;
                call_out("xywar_message", 10, HIG "本次襄陽保衛戰已結束，系統將於1分鐘內釋放資源，襄陽恢復平靜。");
                reset_data(); // 初始化所有數據
        }
                        ...
                        if (random(3) == 1)
                        {
                                switch(random(10))
                                {
                                        ...
                                        case 2:
                                                ...
                                                xywar_message(dj_name + "：哼，兩軍交鋒豈同江湖撕斗，一群烏合之眾必將死於亂箭之下。\n");                                        
                                                break;                
                                        case 3:
                                        ...
                                        case 8:
                                                ...
                                                break;
                                        case 9:
                                                xywar_message("襄陽城四周已經瀰漫著濃烈的硝煙味，城民早已各自收拾細軟，隨時準備趁亂逃命，場面一片混亂。");
                                                break;
                                        default:
                ...
                case 6:
                        ...
                        if (tiemuzhen)
                        {
                                xywar_message(HIG "勝利條件：1、殺死成吉思汗\t       " HIM "失敗條件：1、蒙古人佔領襄陽中央廣場\n" HIG
                                              "\t              2、防守蒙古人攻擊30分鐘\t         " HIM "2、所有參加襄陽保衛戰的玩家剩餘不到五名\n" HIG
                                              "\t              3、全部殲滅蒙古人\n" 
                                              HIG "\t              4、蒙古人退兵\n" NOR);
                        }
                        else
                        {
                                xywar_message(HIG "勝利條件：1、防守蒙古人攻擊30分鐘\t       " HIM "失敗條件：1、蒙古人佔領襄陽中央廣場\n" HIG
                                              "\t              2、全部殲滅蒙古人。\t       " HIM "          2、所有參加襄陽保衛戰的玩家剩餘不到五名\n" 
                                              HIG "\t              3、蒙古人退兵\n" NOR);                                
                        }
