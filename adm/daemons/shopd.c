...
public int do_buy(object obj, object me, string arg)
{
        ...
        if (intp(value))
        {
                ...
                {
                case 0:
                        tell_object(me, CYN + obj->name(1) + NOR + CYN "冷笑道：窮光蛋，一邊獃著去。\n" NOR);
                        return 1;
...
public string do_invite(object ob, object me, string arg)
{
        ...
        if (num == 10)
        {
                ...
                room->save();
                return CYN + ob->name(1) + NOR + CYN "翻開帳簿用划了劃，奸笑道：老闆，已經按照你的吩咐取消掉 " + arg + " 的貴賓資格了。\n" NOR;
        }
...
public string do_ban(object ob, object me, string arg)
{
        ...
        if (sscanf(arg, "%s %s", type, the_id) == 2 && type == "-")
        {
                ...
                set("ban", ban, room);
                room->save();
                return CYN + ob->name(1) + NOR + CYN "翻開帳簿划了劃，對你賠笑道：老闆，已經取消了 " + the_id + " 黑戶身份。\n" NOR;
        }
