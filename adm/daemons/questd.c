...
public int ask_quest(object me, object who)
{
        ...
        if( objectp(aob=query_temp("quest/assist", who)) )
        {
                message_vision(CYN "$N" CYN "看了看$n" CYN "，道：我聽說你不是幫" NOR +
                               HIY + aob->name(1) + NOR + CYN "去了么？先辦妥了"
                               "再說吧！\n" NOR, me, who);
                return 1;
        }
...
int accept_object(object me, object who, object ob)
{
        ...
        if( query("money_id", ob) )
        {
               ...
               {
                       tell_object(who,me->name() + "奇怪的看着你說：我沒給你什麼任務啊？\n");
                       return 0;
               }

               if (ob->value() < 1000000)
               {
                       tell_object(who,me->name() + "不屑一顧的說：這麼一點點錢，能用來幹什麼？你自己留着買雞腿用吧！\n");
        ...
        } else
        {
                ...
                case 0:
                        ...
                        weiwang = weiwang / 3 + 1;
                        score = score / 4 + 1;
                        msg += CYN + me->name() + CYN "看着你接著說道：還行吧，下"
                               "次我給你個難點的任務。\n" NOR;
                        break;
                case 1:
                ...
                if (total_count == 300)
                {
                        msg += CYN + me->name() + CYN "嘆道：“真是長江後浪推前浪，想不到你接連着" + chinese_number(quest_count) +
                               "次任務都無一疏漏，不易，不易！”\n" NOR;
                        special = "/clone/gift/tianxiang";
                } else
                if (total_count == 500)
                {
                        msg += CYN + me->name() + CYN "深深嘆了口氣，道：“想不到你連着" + chinese_number(quest_count) +
...
么不在任務產生的同時由任務自己登記？這是因為任務守護進程有可
