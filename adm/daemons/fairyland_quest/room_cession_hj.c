...
一個別的ID（甚至是新註冊的ID）來進行遊戲，並且：
    ...
    將兩個ID都登記一次，即可相互轉讓在遊戲里得到的 EXP/POT 。
...
*/
    ...
    message_vision( sprintf("$N將 %d 點幻境遊戲獎勵的 %s 兌現成了MUD里的 %s 。\n", amount, type, type), me );
...
int do_checkme()
{
    ...
    if( query("huanjing2003/exp", me)>0 )
        write(HIY"你有"+query("huanjing2003/exp", me)+"點在本遊戲里獲得的經驗值。\n"NOR);
    else write(HIY"你沒有在本遊戲里獲得的經驗值。\n"NOR);

    if( query("huanjing2003/pot", me)>0 )
        write(HIY"你有"+query("huanjing2003/pot", me)+"點在本遊戲里獲得的潛能值。\n"NOR);
    else write(HIY"你沒有在本遊戲里獲得的潛能值。\n"NOR);
