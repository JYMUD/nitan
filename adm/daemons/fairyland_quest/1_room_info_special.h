/*********************************************************
   ...
   其一，殺了各族的族長時，可隨機得到該族的權杖，搜集所有
         族的權杖，再回來這裡發獃 JOB1_NEED_IDLE 秒，即可
         得到大量獎勵。

   其二，在此發獃 JOB2_NEED_IDLE 秒，即得到 幽靈武士 JOB。
         殺了它可以得到額外的獎勵。若被其他人殺去，與普通
         各族的NPC效果一樣。本人殺了幽靈族武士時，可得到
         技能提升與遊戲得分。

   JOB1 的發獃生效時間必須比 JOB2 的短。具體看下邊的設置。

   另外，在此房間停留，會隨機損失 hp ，至死方休！ 

   int query_idle( object target ) 返回發獃值 :)
...
void create()
{
    ...
    set("long", "你剛走過來，卻發現這裡是一小方水潭，水潭之水清幽平靜，潭邊立着一個
...
[42m     [30m   [37;0m
    ...
    // 奈何註：此圖複製自哈工大BBS，顏色設置與其並無二致。亭外岸邊的一豎是我加上的，
    // 為了一些其他的效果。
...
int do_lvup(string arg)
{
    ...
    if( me->is_busy() || query_temp("hj_need_waiting", me) )
        return notify_fail("你還忙着呢，那麼重要的事情還是等有空的時候再說吧。\n");
