...
其中一個樣貌古怪的女子說對着$N說道：“恭喜恭喜！幻境正在發放特別獎勵，你是這次的幸運者！”
...
************************************************************\n",
    ...
    {
        if( temp< 1 || temp > 1000 ) return notify_fail("設置發布信息價格，範圍 1-1000 黃金。\n");
...
int valid_leave(object me, string dir)
{
    ...
    else
    {
        ...
        destruct(menpiao);
        message_vision("$N掏出一張門票塞進門邊的小盒子里，隨即往裡邊走了進去。\n",me);
        in_times++;
        save();
    }
...
void player_timeout(object me)
{
    ...
    message_vision( "$N呆在這裡太久了，忽然感到一陣震蕩！\n",me);
    me->move( this_object() );
    message_vision( "$N被一陣莫名的震蕩送到了這裡。\n",me);
}
