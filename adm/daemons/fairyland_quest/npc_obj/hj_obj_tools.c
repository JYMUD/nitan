...
1、道具太多，玩家將無所適從
2、每次想到新的都要添加，何時才是個頭？
3、一些特強力的道具，可留待 幻境系列 第2集 里寫！

by the way, 第2集的情節已想好幾個國度的構思了！
玩家將能夠進一步深入遊戲中去，繼續延續第1集里
的故事！！！

並且，第2集里將是以情節為主的內容！大家
拭目以待一個迷一般、史詩一般的情節吧！
...
string *longs=({
    ...
    "這是一顆飛翔之石，用它你可以(feixiang / fx)瞬間飛翔到幻境里的任何地方。\n",
    "這是一個脫離水晶，用它你可以(tuoli / tl)逃開怪物，隨機脫離去到別的地方。\n",
    "這是一面回歸之鏡，用它你可以(huigui / hg)立刻離開幻境，回到現實里來（遊戲得分將保持）。\n",
    "這是一顆祈禱之石，用它你可以(qidao / qd)祈禱隨機寶物出現於自己所處之地。\n",
    "這是一個召喚之盒，用它你可以(zhaohuan / zh)召喚隨機精靈出現於自己所處之地。\n",
    "這是一面發獃之旗，用它你可以(fadai / fd)讓同一個地方的人發獃一會兒。\n",
...
int do_dingxiang()
{
    ...
    if( rooms && temp_mark=query("room_mark", rooms) )
    {
        ...
        write(query("name")+"里隱約顯現着一個標記，似乎是 " + chinese_number(temp_mark) + " 。 \n");
    }
    else
...
void fadai( object target, int time )
{
    ...
    {
        message_vision( CYN"$N"NOR+CYN"身形忽隱忽現，竟似並無實體一般，發獃之旗卻是毫無用處。\n"NOR, target );
    }
    else
    {
        message_vision(HIY"只見$N"HIY"忽地雙目發直，果然就是個獃子般模樣。\n"NOR,target);
...
int do_fadai(string arg)
{
    ...
    //if( hj_busy_now( me ) )
        //return notify_fail("你還有別的事情忙着呢！\n");
    if(!arg)
        return notify_fail("你要讓誰發獃？格式：\n"
            "讓某人發獃<fadai"+query("id", me)+">\n"
            "讓這個地方的所有有氣息的人發獃 <fadai all> (需要消耗自身的氣息)\n");
    ...
    if(arg != "all")
    {
        ...
        if( !query_temp("hj_hp", target) || query_temp("hj_hp", target)<1 )
            return notify_fail(query("name", target)+"沒有幻境里的氣息，無法對其進行攻擊。\n");
        if(!living(target))
            return notify_fail("這個遊戲里必須是清醒的對手才能攻擊。\n");
        if( target == me )
            return notify_fail("讓自己發獃？沒必要吧……\n");
        message_vision(HIY"$N"HIY"舉起"NOR"$n"HIY"大聲叫道：“獃子出現！”\n"NOR,me, ob);
        fadai( target, time );
    }
    else
    {
        ...
        if( human_amount < 1 )
            return notify_fail("這兒沒別的人，這可是個寶貝，還是省着點用吧。\n");
        message_vision(HIY"$N"HIY"舉起"NOR"$n"HIY"聲嘶力竭般大聲叫喊道：“一堆獃子，快快出現！”\n"NOR,me, ob);
        for(i=0;i<sizeof(inv_here);i++)
        {
            if( query_temp("hj_hp", inv_here[i]) && 
                query_temp("hj_hp", inv_here[i])>0 && 
...
int do_xunren(string arg)
{
    ...
    write(query("name")+"里隱約顯現着出標記，似乎是 "+chinese_number(temp_mark)+" 。\n");
    if( query("use_times") <=0 )
    {
        remove_call_out("delete_me");
        call_out("delete_me",1);
...
int do_kuitan(string arg)
{
    ...
    if( !query_temp("hj_hp", target) || query_temp("hj_hp", target)<1 || 
       query_temp("huanjing", target) != "start" )
        return notify_fail(query("name", target)+"已沒有絲毫幻境里的氣息，算了吧。\n");
    if(!living(target))
        return notify_fail("這個遊戲里必須是清醒的對手才能進行操作。\n");
