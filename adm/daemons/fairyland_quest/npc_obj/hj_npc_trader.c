...
void create()
{
    ...
    set("long", "這是一個專做代售生意的商人，幻境里勇士們之間的交易都是到這裡來進行的。
...
發布求購信息後，將於半小時後清除；或在此期間若有該ID的新貨到來，將在通知後清除。\n\n");
...
// 這個文件里許多描述都不用  message() ，是不想對其他人造成太多的騷擾
...
int do_quhui( string arg )
{
    ...
    g->move(me);
    message_vision( sprintf( CYN"$N"NOR+CYN"對着$n"NOR+CYN"說道：“我想取回我的%s"NOR+CYN"。”\n$n"NOR+CYN"點了點頭，拿出一%s%s"NOR+CYN"遞給了$N"NOR+CYN"。\n"NOR, 
...
int do_goumai( string arg )
{
    ...
    g->move(me);
    message_vision( sprintf( CYN"$N"NOR+CYN"對着$n"NOR+CYN"說道：“我想買個%s"NOR+CYN"。”\n$n"NOR+CYN"點了點頭，拿出一%s%s"NOR+CYN"遞給了$N"NOR+CYN"。\n"NOR, 
        query("name", g),query("unit", g),query("name", g)),me,this_object());
    me->start_busy(1);
    ...
    if( environment(seller) == environment(this_object() ) )
        message_vision( CYN +query("name")+NOR+CYN"對着$N"NOR+CYN"說道："
...
void new_goods_in( string id )
{
    ...
    for(i=0;i<sizeof(ids);i++)
    {
        ...
        if( environment(who) == environment(this_object() ) )
            message_vision( CYN"$n"NOR+CYN"對着$N"NOR+CYN"說道：“嘿，你不是說要買"
                +id+"嗎，現在有貨了。”\n"NOR,who, this_object() );
        else
            message_vision( CYN"只見一個"+query("name")+NOR+CYN"匆匆走了過來，對着$N"
