void create()
{
    ...
    set("no_drop","這東西是只屬於你一個人的。\n");
    set("no_give","這東西是只屬於你一個人的。\n");
    set("no_steal","這東西是只屬於別人一個人的。\n");
...
//    write("checking.\n");
    ...
    if( wait_over !="yes" && time > DENG_TIME )  // 時間到，結束遊戲（ 不算 fail ）
    {
        ...
        else
        {
            ...
            if( deng > 0 )
                message_vision("$N只聽得似有一聲微響，只見"+query("name", ob)+"里的一個亮點熄滅了。\n",me);
            else message_vision("$N只聽得似有一聲微響，只見"+query("name", ob)+"里的最後一個亮點也熄滅了。\n",me);
            ...
            longs="這是一個四四方方的小盒子，盒蓋透明，你能見到內里有"+long_color+chinese_number(deng)+"個小點"NOR"在散發著微弱的光芒。\n";
            set_name(long_color+"七星燈"NOR, ({"qixing deng","deng"}));
    ...
    set("lost_time",time);
    set("last_deng",deng);
    // 更新後的走路忙時系統，配合房間里的增加busy語句，這裡就減少。
...
void game_over()
{
    ...
    me->move( HJ_OUT_ROOM );
    message_vision(HIR"$N"HIR"身影逐漸顯現，似是回到現實里來了。\n"NOR,me);
...
void sing_song(int i, object me)
{
    ...
    else
    {
        ...
        write(HIW"在幻境里，你只能使用特別的指令來進行攻擊。詳細請用 <help huanjing> 指令查詢。\n"NOR);
...
void enter_hj()
{
    ...
    message_vision(HIR"$N"HIR"踏步走出，忽然間感到一陣強烈的震蕩！……\n"NOR,me);
    ...
    // 得分，這個是遊戲結束後重要獎勵依據之一，在遊戲里由各種途徑增加
