...
int do_found( string arg )
{
    ...
    message_vision(CYN"$N"NOR+CYN"興奮地拿出一"+query("unit", f_obj)+f_obj->name(1)+NOR+CYN"，對着$n"NOR+CYN"說道：“你是不是在找這個？”\n",me,ob);
...
int do_huida(string arg)
{
    ...
    if( me->is_busy() || query_temp("hj_need_waitting", me) )
        return notify_fail("你還忙着呢，空閑點了再接受提問吧。\n");
    ...
    // 若玩家在本輪遊戲里，已把5個預設的問題都答對過了，將不再發放問題。
        ...
        {
            case "int":
            command("say “這其中一人，領悟之力奇高，因此過人一籌，終於了其心愿，\n          成就大事。如此之人，當稱英雄！”");
            break;
            case "str":
...
int do_lianli()
{
    ...
    if( me->is_busy() || query_temp("hj_need_waiting", me) )
        return notify_fail("你還忙着呢，還是等空閑了，專心點來煉力吧！\n");
    ...
    command("whisper "+id+" 這樣這樣，這樣這樣……明白了么？");
...
int do_answer(string arg)
{
    ...
    if(arg == "ok")
    {
        ...
        else
        {
            ...
            set("cannot_rdmove","yes");
            message_vision("$N對着"+query("name")+"大聲說道：“我帶你去吧！”\n",me);
    ...
    if(arg == "byebye")
    {
        ...
        if( query("id", me) != query("following_id") )
            return notify_fail("又不是你答應帶她去的，你愛走便走么。\n");

        message_vision("$N撓了撓腦袋，對着"+query("name")+"說道：“我帶不了你去了，就這樣吧，我走了。”\n",me);
...
int do_glist()
{
    ...
    // yeah 這下不怕了，obj_copy.h 里的映射變量亂改都沒問題！哈哈哈
    // 8:29 02-10-30
