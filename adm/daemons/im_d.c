...
void login_irc(object user)
{
    ...
    if(!id)
    {
        tell_object(user,"你有登入 ID ");
        return ;
    }
    fd = socket_create(STREAM, "read_callback","close_socket");
    if ( fd < 0 )
    {
        tell_object(user,"失，可能是主有或是路法。");
        return;
    }
    ...
    {
#ifdef DEBUG
    TELL(sprintf("[%s] %s 成功 Socket ,始登入 irc.fd = %d\n",
         ctime(time()) ,id,fd));
#endif 
    }
    else
        ...
        default:
		    ...
		    m = replace_string(m,"account :","account:");
                    if(sscanf(m,":%s!%s PRIVMSG account:%s",name,mail,msg) == 3 )
                    {
                        ...
                        {
                            tell_object(users[fd]["obj"],"\n[IM Message]:"+mail+"想要把你加入他的好友名，同意的用 im yes，不同意的，用 im no。\n");
                        }
...
//                            case "MSN - Logging in: Authenticated, getting buddy list":
                    ...
                    // 上站息 :paiting!paiting@hotmail.com JOIN :&bitlbee
			...
			CH_D->register_relay_channel("qq");
                    ...
                    // 下站息 :paiting!paiting@hotmail.com QUIT :Leaving...
...
//    i = efun::socket_write(fd,message);
    switch(i)
    {
        ...
        case EESECURITY:      TELL("企反安全。"); break;
        case EENOADDR:        TELL("socket 未指定位址。"); break;
        case EEBADADDR:       TELL("位址格式的。"); break;
        case EENOTCONN:       TELL("socket 尚未接。"); break;
        case EEALREADY:       TELL("操作已在行中。"); break;
        case EETYPENOTSUPP:   TELL("不支援此物件型。"); break;
//        case EEBADDATA:       TELL("送出的料含有太多巢次 (nested level)。"); break;
        case EESENDTO:        TELL("sendto 的。"); break;
        case EEMODENOTSUPP:   TELL("不支援此 socket 型。"); break;
        case EEWOULDBLOCK:    TELL("操作停 (block)。"); break;
...
void process_who(int fd,string str)
{
    ...
    //:rw.twku.net 352 whatup whatup_ whatup mail2000.com.tw rw.twku.net whatup_ H :0 缸-有要 gmail ，我有一堆-.
...
int process_send_msg(object me,string who,string msg)
{
    ...
    if(undefinedp(users[fd]) ||
       undefinedp(users[fd]["list"][who]))
        return notify_fail("[IM Message] 即息上的好友名有人！\n");
//	socket_write(fd,"PRIVMSG &bitlbee :set charset 'utf-8'\r\n");	
...
//		socket_write(fd,"PRIVMSG &bitlbee :set charset 'gb2312'\r\n");	
    {
        ...
        if (n[4]&&n[0]&&n[1])
        	tell_object(me,sprintf(HIW HBBLU"你用即息 %s : %s\n"NOR,
            	      n[4]+"("+n[0]+"@"+n[1]+")\n["+who+"]",msg));
    }
    return 1;
}
