...
string  *mail_reg; // 這裡保存已註冊的Email地址。
...
void register_mail(object user,string mail)
{
        ...
        if(strsrch(mail,',') >= 0)
        {
        write(sprintf("你的電子郵件地址：%s 里包含非法字符，\n請認真檢查後重新註冊。\n",mail));
        return;
        }
...
void send_mail(object user, string mail_from, string mail_to, string topic, string data)
{
        ...
        {
                write(sprintf("電子郵件地址：%s 里包含非法字符，無法發送。\n", mail_to));
                return;
        }
...
// 下面的這個函數時我加上為了處理48小時內已經登陸的玩家，從未註冊用戶列表中刪除這些玩家。
