...
void create()
{
        set_name("葯臼", ({ "yao jiu", "jiu" }) );
        set("long",
                "這是一個葯臼，是醫士們用來煉製丹藥的容器：\n"
                "put <藥材>          將藥材放入葯臼中。\n"
                "fill                向葯臼中添加清水。\n"
                "burn                點火折燃燒木柴。\n"
                "putout              滅火冷卻葯臼。\n"
...
int do_fill(string arg)
{
        ...
        if (!arg || arg != "water")
                return notify_fail("你要往葯臼里添加什麼？\n");
        if (query("process") > 0)
                return notify_fail("你已經往葯臼里添過水了啦！\n");
        if( !query("resource/water", room) )
                return notify_fail("這裡沒有水。你找個有水之處再添水吧。\n");
        set("process", 1);
        message_vision("$N往葯臼里添了些清水。\n", this_player());
        return 1;
}
int do_burn()
{
        ...
        {
                if (query("process") == 0)
                        return notify_fail("還沒放水就點火，想干燒啊！\n");
                if (query("process") != 1)
        ...
        set("process", 2);
        set("burntime", time());
        message_vision("$N掏出火折一晃，火折亮了起來。火折點燃木柴後，再把葯臼放在柴上。\n", this_player());
        for ( seq = 0; seq < sizeof(inv); seq++)
        {
                if( !query("vegetable", inv[seq]) )
                {
                        set("process", 3);
                        message_vision("葯臼里混進不能入葯的東西。看來這臼葯沒用了。。。\n", this_player());
                }
                i+=query("vegetable", inv[seq]);
                j+=query("nostrum", inv[seq]);
                destruct(inv[seq]);
        }
        message_vision("過了一會葯臼里冒出一股淡淡的白氣。藥材漸漸熬到水中。。。\n", this_player());
        set("vegetable", i);
        set("nostrum", j);
        return 1;
}
int do_putout()
{
        ...
        if (query("vegetable") == 0)
                return notify_fail("葯臼里沒有藥材，熬不成藥了！\n");
        if (query("process") < 2)
                return notify_fail("現在沒有火可滅了啦！\n");
        if (time() - query("burntime") < 180 && !wizardp(this_player()))
                return notify_fail("葯正在熬，火候還沒到，現在可滅不得火！\n");
        if (time() - query("burntime") > 300 && !wizardp(this_player()))
        {
                set("process", 0);
                return notify_fail("葯好像熬了太久，火候過了，葯熬糊了！\n");
        }
        message_vision("$N撤了着火的木柴，讓葯臼自行冷卻。\n", this_player());
        if (query("process") == 3)
        {
                set("process", 0);
                return notify_fail("肯定是葯臼中摻進雜質了，這次葯沒熬成！\n");
        }
        set("process", 0);
