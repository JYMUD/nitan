...
int do_join(string arg)
{
        ...
        if( query("is_vendor", me) )
                return notify_fail(CYN+query("name", ob)+"皺了皺眉，說道：你不是已經加入商業協會了么？\n"NOR);
        if( query("bad_vendor", me) )
                return notify_fail(CYN+query("name", ob)+"冷笑一聲，說道：既然你已經脫離了商業協會，又回來作甚？\n"NOR);
        tell_object(me,CYN+query("name", ob)+"點了點頭，說道：你真的打算("HIY"decide"NOR+CYN")加入商業協會么？可考慮清楚了？\n"NOR);
...
int do_unjoin(string arg)
{
        ...
        tell_object(me,CYN+query("name", ob)+"嘆了口氣，說道：如果脫離商業協會，以後可不能反悔。你真打算("HIY"decide"NOR+CYN")這麼做么？\n"NOR);
...
int do_decide()
{
        ...
        {
                message_vision(HIC "$N" HIC "對着$n" HIC "微笑道：如今世道錢乃萬能之物，以後好好乾吧。\n" NOR, this_object(), me);
...
int do_yao()
{
        ...
        if( query_temp("shang-ling", me) )
                return notify_fail(CYN+query("name", ob)+"皺眉道：剛才我不是才給了你一張么？那麼快就弄丟了？\n"NOR);

        if( SHOP_D->is_owner(query("id", me)) )
                return notify_fail(CYN+query("name", ob)+"微笑道：你現在不是已經開店了么？還要這個幹嘛。\n"NOR);
