...
int do_auction(string arg)
{
        ...
        if (query("now_shop"))
                return notify_fail("現在正在進行" + query("now_shop") +"店鋪的拍賣，您別著急。\n");
...
int begin_auction()
{
        ...
        case 1:
                message_vision(CYN"$N" CYN "皺了皺眉頭，說道：有人願意購買么？請即時舉牌(" HIY "jupai" NOR + CYN ")。\n" NOR, operator);
                break;

        case 2:
                message_vision(CYN"$N" CYN "微笑道：各位考慮好了么？有人願意舉牌(" HIY "jupai" NOR + CYN ")出價么？\n" NOR, operator);
                break;
...
int do_jupai(string arg)
{
        ...
        if (! query("now_shop"))
               return notify_fail(operator->name(1) +"還沒有宣布要拍賣哪家店鋪，你急個啥？\n");
        ...
        if( query("balance", buyer)<(money+now_price)*10000 )
                return notify_fail("你帳戶里的錢並不足" + chinese_number(money+now_price) + "兩黃金。\n");
        ...
        case 1:
                message_vision(CYN"$N" CYN "微笑道：有人願繼續拼價么？請有意者繼續舉牌(" HIY "jupai" NOR + CYN ")出價。\n" NOR, operator);
                break;
...
int do_qiao()
{
        ...
        if (m = query("wait_jupai"))
        {
                ...
                } else return notify_fail("您準備繼續宣布拍賣哪個店鋪？\n");
