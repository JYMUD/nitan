...
string ask_fenglu()
{
	...
	command ("addoil " + ob->query("id"));
	ob->add("balance", gold);
	return "你本月的俸祿"+MONEY_D->money_str(gold)+"已經存入錢莊了，但願這位"+ RANK_D->query_respect(ob) +"不要使我失望，好好乾吧！";
}
...
mixed ask_bo()
{
	...
	message_vision(
		"$N眼中發出兩道寒光直射$n，一字一字地說道：家父現在身處少林，那裡藏龍卧虎，\n"
...
int do_jiaoliang()
{
	...
	if (obj->query_temp("marks/jiao")) {
		...
		message_vision(
			"$N對着$n冷冷說道：你武功未成，就好高騖遠，你還是安心練功去吧！\n",
			me, obj
		);
...
*/
	...
	message_vision(CYN"慕容復仔細研究着$N記在紙卷上的武功精要，情不自禁地說道：好一個"+me->query_temp("skill_target")+"。\n"NOR, me);
