...
void create()
{
        set("short", "聚賢庄大廳");
        set("long", @LONG
這是聚賢庄的大廳，大廳內很寬敞，放着幾十張桌子。東首主位的桌子旁
放有三把黑色的太師椅，其中一個椅子上坐着一個中年男子。他身後的牆上掛
有兩副圓盾、一柄單刀，一桿短槍。
LONG
        );
		set("quest",1);
...
void init()
{
  ...
  tell_object(me,HIG"\n你走進大廳內，果然一片混亂。大廳上聚集着三百餘人，大家擠在一團，真能挨到蕭峰身邊的，不過五六人而已，\n"
  ...
  log_file("quest/TLBB", sprintf("%s(%s)聚賢庄大混亂開始。經驗%d。\n", me->name(1),me->query("id"), me->query("combat_exp")) );
...
void xiaofeng2(object me)
{
  ...
  message_vision(HIW"\n$N見他攻到，兩隻寬大的衣袖鼓風而前，便如是兩道順風的船帆，威勢非同小可，大聲喝道：“袖裡乾坤，\n"
                      ...
                      "力道相互激蕩，突然間大廳上似有數十隻灰蝶上下翻飛。\n" NOR, obj1,obj2); 
  tell_object(me,HIB"\n你暗叫一聲“好武功”，蕭峰只如此一掌，便破了渡難的成名絕技。\n"NOR);  
  log_file("quest/TLBB", sprintf("%s(%s)聚賢庄大混亂玄難與蕭峰單幹。經驗%d。\n", me->name(1),me->query("id"), me->query("combat_exp")) );
...
void xiaofeng4(object me)
{
  ...
  log_file("quest/TLBB", sprintf("%s(%s)聚賢庄大混亂需要解救蕭峰。經驗%d。\n", me->name(1),me->query("id"), me->query("combat_exp")) );
...
void xiaofeng5(object me)
{
  ...
  copy_person(this_object(),3);//複製情節人物 
  tell_object(me,HIR"\n你趕快去解救蕭峰（jiejiu xiao feng）啊。別發獃了！\n"NOR);
  remove_call_out("xiaofeng5");
  call_out("xiaofeng5",10, me); 
  return;
}
void xiaofeng_over(object me)
{
  ...
  me->add("mana",-200);
  if(me->query("mana")<0) me->set("mana",0);
  log_file("quest/TLBB", sprintf("%s(%s)聚賢庄大混亂沒有解救蕭峰，任務失敗。經驗%d。\n", me->name(1),me->query("id"), me->query("combat_exp")) );
  destruct_person(this_object());//消除所有人物
  return;
}
...
int do_jiejiu(string arg)
{
  ...
  else 
  {
	  ...
	  me->delete_temp("quest/busy");//任務系統衝突標誌
	  me->delete_temp("quest/天龍八部");   
	  log_file("quest/TLBB", sprintf("%s(%s)聚賢庄缺少houyuan.c文件。\n", me->name(1),me->query("id")) );
	  return 1;
  }
  ...
  else 
  {
	...
	me->delete_temp("quest/天龍八部");   
    tell_object(me,HIR"\n你跟着又甩長繩，再繞遠處大樹，如此幾個起落，靠，還在原地？？\n"NOR);
	log_file("quest/TLBB", sprintf("%s(%s)聚賢庄缺少duanya2.c文件。\n", me->name(1),me->query("id")) );
    return 1;
  }
  log_file("quest/TLBB", sprintf("%s(%s)聚賢庄大混亂成功解救蕭峰。經驗%d。\n", me->name(1),me->query("id"), me->query("combat_exp")) );
