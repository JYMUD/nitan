...
void create()
{
	...
	set("long", "他是一位白須白眉的老僧，身穿一襲金絲綉紅袈裟。\n"
...
void attempt_apprentice(object ob)
{
	...
	if ( ob_fam["generation"] == (my_fam["generation"] + 1) && name[0..1] == "澄") {
		command("say " + ob_fam["master_name"] + "的徒弟怎麽跑到我這兒來了，哈哈哈 !");
...
int ask_job()
{
	...
	if(!wizardp(me)&& me->query("combat_exp") >= me->query("slexp") && me->query("combat_exp") <= me->query("slexp") + 500){
		command("say 你武功精進也太慢了，老衲怎麼放心讓你去干啊。");
		return 1;
	}
	...
	if( me->query("shen") < 1 ){
		command("heng");
		command("say 這位" + RANK_D->query_respect(me)+"眼露凶光，我少林名門正派，可不敢勞你大架。");
		return 1;
	}
...
int ask_zhangxing()
{
	...
	if(me->query(QUESTDIR3+"bad") && me->query(QUESTDIR5+"start")&& me->query_temp(QUESTDIR5+"yeerniang")&& me->query_temp(QUESTDIR5+"zuinie")&& !me->query_temp(QUESTDIR5+"zhangxing")&& !me->query(QUESTDIR5+"over"))
	{
	  ...
	  message_vision(HIY"\n$N朗聲說道：“不錯，老衲犯了佛門大戒，有傷鸛林清譽。國有國法，家有家規。自來任何門派\n"
	                    ...
	                    "譽攸關，不得循私舞弊。”說著跪伏在地，遙遙對着佛像，自行捋起了僧袍，露出背脊。\n"NOR,this_object());
...
int ask_xiaofeng()
{
	...
	if(me->query(QUESTDIR3+"bad") )
  {
	...
	if(!me->query_temp(QUESTDIR4+"askxuanci"))
	{
		command("whisper "+me->query("id")+" 如果當真，少林自當儘力，當年少林與蕭家誤會頗多，這也是我中原武林贖罪的一次良機。\n"NOR);
