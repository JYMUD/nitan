...
void greeting(object me)
{
	...
	if(me->query(QUESTDIR3+"start") && me->query_temp(QUESTDIR3+"lookcliff")&& me->query_temp(QUESTDIR3+"over") && !me->query(QUESTDIR3+"over"))
	{
		...
		tell_object(me,HIG"\n\n你走過去看了看蕭峰，還是將你看到的告訴了蕭峰：“智光大師所道的山壁上如今正中一大片儘是斧鑿的印痕，\n"
			"顯而易見，是有人故意將留下的字跡削去了。”\n"NOR);
...
int ask_truth()
{
    ...
    if(me->query(QUESTDIR3+"start")&&me->query_temp(QUESTDIR3+"help")&&!me->query_temp(QUESTDIR3+"askxiao"))
    {
    	...
    	command("sigh");
    	command("say 我受傷未能遠行，還請這位"+RANK_D->query_respect(me) +"去一次頌摩崖，那裡據說有一塊刻字記載當年事迹。");
    	command("shout");
    	command("thank "+me->query("id"));
...
int ask_yintui()
{
    ...
    {
        remove_call_out("check_answer");
    	tell_object(me,HIW"你見蕭峰獃獃發矇，知道他正為這契丹人的身份傷神，卻聽蕭峰越來越暴躁，大聲號叫：“我不是漢人，我不是漢人！\n"
    	                  "我是契丹胡虜，我是契丹胡虜！”提起手來，一掌掌往山壁上劈去，壁上石屑四濺。喬峰心中郁怒難伸，仍是一掌掌的\n"
