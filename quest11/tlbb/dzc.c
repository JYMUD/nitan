...
string ask_save()
{
  ...
  if(!me->query(QUESTDIR1+"start" ))
  {
	...
	{
		if ( me->query("shen") < 0)
		           return ("我看" + RANK_D->query_respect(me) +"面帶陰邪之氣，恐怕是四大惡人一夥的姦細。");
	  ...
	  return ("既然"+ RANK_D->query_respect(me) +"有意相助，定會如虎添翼。居我得知，譽兒被四大惡人之首\n"+
			...
			"再向西行"+chinese_number(j)+"里，再向北行"+chinese_number(i)+"里，然後會到一座高山，再往後就要見機行事了。\n\n"+
...
string fenglu()
{
	...
	return "這是你本月的俸祿，這位"+ RANK_D->query_respect(ob) +"好好乾啊！";
}
