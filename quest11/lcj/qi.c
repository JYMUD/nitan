...
int ask_xuexi4()
{
	...
	if(ob->query_temp("jobask") == 4) {
		command("say  這位" + RANK_D->query_respect(ob) +"來武館多久啦？在這兒練功之餘可和別人閑聊 "HIY HBCYN"chat"CYN"\n"+
			"來交流交流感情。還可以散布些謠言 "HIY HBCYN" rumor"CYN" ,同一門派的還可以用 "HIY HBCYN" party"CYN"。\n"+
			"閑聊和散布謠言時可用些已經有的動作。你可以試試 "HIY HBCYN"chat* hi"CYN" 。用 "HIY HBCYN"semote"CYN"\n"+
			...
			"打殺殺的，然而我覺得還是閑聊交友更有趣。你好自為之吧。"NOR);
	...
	if ( ob->query_temp("jobover") == 4) {
		command("say 我不是都和你說了么，我師兄有事找你，快去看下吧。");
		return 1;
	}
}
string ask_diyun()
...
string ask_tangshi()
{
	...
	command("consider");
	message_vision(HIY"$N仔細在腦海里搜索“唐詩選輯”。\n", ob);
	command("look "+me->query("id"));
...
string ask_xieyang()
{
	...
	command("en");
	message_vision(HIY"$N慢慢在腦海里搜索“唐詩選輯”、“鞋樣”，突然似乎想了起來。\n", ob);
	command("look "+me->query("id"));
