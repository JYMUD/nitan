...
string quest_feihu(object me)
{
        ...
        if(query("quest/雪山飛狐/寶藏/yaoku", me))
                str +=get_string(YEL"你發現了闖王葯庫，並在葯庫獲得增長內力的神奇靈藥。"NOR);
        ...
        if(query("quest/雪山飛狐/武功/lengquanshengong",me))
                str +=get_string(YEL"你竟然從一顆靈藥里找到一個紙團，並發現冷泉神功秘籍的地點。"NOR);
        else if(query("quest/雪山飛狐/寶藏/shuku",me)>=3)
                str +=get_string(YEL"你從一顆靈藥里找到一個似乎是修鍊武功的紙團，目前只是未得要領。"NOR);
        str +=feihu_skill(me);
        return str;
}
string feihu_skill(object me)
...
string quest_lchj(object me)
{
        ...
        if(!query("quest/連城訣/雪谷激斗篇",me))
        {
                str +=get_string(HBRED"你正在努力中........"NOR);
                str +=lchj_skill(me);
                return str;
        }
        if(!query("quest/連城訣/雪谷激斗篇/over",me))
        {
...
string lchj_skill(object me)
{
        ...
        if(query("quest/連城訣/雪谷激斗篇/start",me))
        {
                str =   "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n";
...
string quest_tlbb(object me)
{
        ...
        if(!query("quest/天龍八部/蕭峰身世篇/over",me))
        {
                ...
                else          str +=get_string(HBRED"竹林驚變，聚閑庄大戰，頌摩崖遺魂，歷史還我真相..."NOR);
                str +=tlbb_skill(me);
                return str;
        }
        str +=get_string(YEL"聚閑庄，你解救了蕭峰成功脫離現場；"NOR);
