...
void greeting(object me)
{
  ...
  {
  	tell_object(me,HIG"\n你走進大廳，卻看見苗人鳳正面向胡一刀的靈牌發獃，你不由一陣感觸，當年幾多周折，今天也算真相大白。\n"NOR);
...
void zhenxiang(object me)
{
  ...
  {
  	command("say");
  	tell_object(me,HIG"苗人鳳果然心思敏捷，你沉默了一下，腦子裡一邊盤想着如何開口詢問這20年前的故事......\n"NOR);
...
int ask_miaojiajianfa()
{
  ...
  if(me->query(QUESTDIR4+"miaojiajianfa_fail")>=3 && me->query("registered")<3)
  {
    ...
    message_vision(HIC"\n\n猶如電光雷閃，一晃數招已過，$n微笑着對$N說道：「剛才我使用的招數可看清楚了？」\n"NOR,me,ob);
    remove_call_out("thinking");
		call_out("thinking",0, me, ob);
    tell_object(me,YEL"你拚命地回憶瞬間交手的一幕，回憶着剛才的招數，你開始認真思索着.........\n"NOR);
    me->start_busy(2);
	  return 1;
}
...
int ask_hujiadaofa()
{
  ...
  if(me->query(QUESTDIR4+"hujiadaofa") && me->query(QUESTDIR4+"miaojiajianfa") )
  {
     ...
     if(me->query(QUESTDIR4+"combat_exp") && me->query("combat_exp")-me->query(QUESTDIR4+"combat_exp")<500000)
  	 ...
  	 {
      	command("say 你已經刀劍融合，可惜缺少內功支持。傳說：冷泉神功可能在闖王寶藏里！");
	      return 1;
      }
      me->set(QUESTDIR4+"time",time());
      me->set(QUESTDIR4+"combat_exp",me->query("combat_exp"));
            ...
            &&QUEST_D->questing(me, 1, 0, 1))
      {
       ...
       {
           tell_object(me,HIG"\n你聽了苗人鳳的指點，腦海里似乎抓住了什麼，可是依然不是很明白。\n"NOR);
           message_vision(HIC"\n$N搖了搖頭，道：“好像還是很困難的。”說罷，深深地嘆了口氣。\n"NOR,me);
           command("poor "+me->query("id"));
...
int ask_zhiliao()
{
  ...
  if(me->query(QUESTDIR2+"over"))
  {
	  tell_object(me,HIW"你也將真相告訴苗人鳳，顯然現在不是悲傷的時候，這毒性越發得厲害。你說服苗人鳳，親自上前仔細觀察中毒癥狀。\n\n"NOR);
  ...
  command("say 藥王的唯一親傳弟子，前段時間藥王病逝，恐怕葯術一派只有這程靈素程姑娘的天地了。");
	return 1;
}
int ask_miaoruolan()
{
