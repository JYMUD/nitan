...
string ask_jindi()
{
       ...
       applicant=this_player();
        ...
        return "我已經給過你了，為何還要向我要？此葯很難煉製，不要太貪心！\n";
        ...
        return "你上次答應我的事情還沒做，怎麽又來要？ 程靈素皺了皺眉，
冷冷的說到：“丹到還有一顆，你要就拿去吧。\n";
        }
...
進去幫我尋得雪蛛，我願將此葯贈於你，你考慮一下，是否答應(yes)或者不答應(no)我。\n";
...
int accept_object(object who, object ob,object me)
{
	 ...
	 if(ob->query("id")=="qixing haitang" )
   {
     ...
     if(who->query_temp(QUESTDIR3+"qixinghaitang") && who->query(QUESTDIR3+"start")&& ob->query("owner")==who->query("id"))
     {
     	...
     	command("say 就是它！但是要想製造解藥，還需要一種東西，它就生長在五毒教，一種藍色的葯花。");
...
void giveyaofang(object me)
{
      ...
      if(!me->query_temp(QUESTDIR3+"getjieyao")) return;
	    if(environment(me)!=environment(this_object()))
     	...
     	command("say 等着急了吧。這是藥方，你可以使用內屋那個葯爐製作。");
      obj->move(me);
      tell_object(me,HIC"程靈素給你一份"+HBCYN"藥方"NOR+HIC"。\n"NOR);
  	  ...
  	  if(!present("lanse yaohua",me))
  	  {
  		  ...
  		  obj->set("no_drop",1);
  		  obj->move(me);
        tell_object(me,HIC"程靈素給你一朵"+HBCYN"藍色葯花"NOR+HIC"。\n"NOR);
  	  }
...
int ask_jieyao()
{
  ...
  {
	  command("nod "+me->query("id"));
	  command("say 除了七星海棠，還需要五毒教一種藍色葯花，我們一起找找看。");
	  return 1;
  }
  ...
  {
	  command("nod "+me->query("id"));
	  command("say 七星海棠正是這主葯之一，可惜被閻基和石萬嗔搶走了。你趕快去找回來吧。");
	  return 1;
  }
  if(me->query(QUESTDIR3+"start"))
  {
    tell_object(me,HIC"你着急地將苗大俠病情和癥狀轉告給程靈素，應該是斷腸草之毒。\n"NOR);
    tell_room(environment(me),HIC"\n"+me->query("name")+"很着急地和程靈素說了些什麼。\n"NOR, ({ me }));
	  command("nod "+me->query("id"));
	  command("say 苗大俠也是我敬佩的人，我自當儘力。你說這種毒藥的確很兇猛，雖然不是立刻斃命，但是必須使用一種奇草。");
