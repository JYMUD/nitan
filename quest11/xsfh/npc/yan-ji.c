...
int ask_biwu()
{
  ...
  if(me->query(QUESTDIR1+"over"))
  {
  	...
  	command("say 但是胡大俠當年豪氣衝天，可惜小人苗人鳳竟然使用毒劍，這段公案遲早有人要去還的。");
	  return 1;
  }
	command("heng");
	command("say 胡大俠當年豪氣衝天，可惜小人苗人鳳竟然使用毒劍，這段公案遲早有人要去還的。");
	return 1;
}
	...
	if(ob->query("quest/try_fight"))
	{
      ...
      if(me->query(QUESTDIR1+"start")&& me->query_temp(QUESTDIR1+"liangye"))
      {
           ...
           tell_object(me,HIC"\n\n卻說當年比武的第六天，胡一刀之妻竟然發現苗人鳳的劍法破綻，按那劍法，苗人鳳右手一劍斜刺，左手上揚，就與白鶴將\n"
        	                       ...
        	                       "忙叫道：『"HIR"苗兄，不可"HIC"！』\n"NOR);
                 tell_room(environment(me),HIC"只見，閻基附在"+me->query("name")+"身邊，低聲說著什麼，神色有些恍惚，似乎在回憶着往事，說話有時一頓一頓的。\n"NOR, ({ me }));
        	 command("sigh");
        ...
        if(me->query(QUESTDIR3+"start")&& me->query_temp(QUESTDIR3+"jieyao1"))
        {
           ...
           obj->set("owner",me->query("id"));
 	         obj->move(me);
 	         tell_object(me,HIR"\n你上前喝道：“"+ob->query("name")+"，那也不能饒你。”當即內力一松，"+ob->query("name")+"竟然暈了過去，你順勢跟上一掌。\n"NOR);
