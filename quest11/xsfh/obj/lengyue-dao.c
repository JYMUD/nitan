...
void create()
{
        ...
        if (clonep())
                ...
                set("unit", "柄");
                set("long", HIB"刀光如水，在冷月下流轉不定。刀柄上用金絲銀絲鑲着一鉤眉毛彎月。\n"NOR);
                set("value", 1);
                set("unique", 1);
                set("rigidity", 4);
                set("sharpness", 4);
...
int do_search()
{
     ...
     if((string)file_name(environment(me))!=me->query_temp(QUESTDIR5+"map_where"))
	    return notify_fail(HIW"你繞着"+environment(me)->query("short")+HIW"走了一圈，這裡沒有寶藏！\n"NOR);
     me->start_busy(2);
     ...
     me->add_temp(QUESTDIR5+"search",1);
      ...
      if(me->query_temp(QUESTDIR5+"search")==4||me->query_temp(QUESTDIR5+"search")==8)
      {
        ...
        message_vision(HIC"突然從角落裡跳出一個人，$n對着$N嚷道：“我就是"+obj->query("title")+HIC"，趕快離開"+environment(me)->query("short")+HIC"。”\n"NOR, me,obj);
