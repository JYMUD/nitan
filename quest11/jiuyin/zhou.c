...
void create()
{
	...
	set("inquiry", ([
		"name": "叫我周伯通就好了。",
		"rumors": "我被那個臭東邪關在這裡十幾年，每天就是一個啞仆來送飯。",
		...
		"南帝": "老頑童做事對不起他，沒臉再見他了。",
		"老頑童": "我就是啊，找我有什麼事嗎？",
		"空明拳": "空明拳共七十二路，口訣是：空朦洞松、風通容夢、沖窮中弄、童庸弓蟲。",
		"kongming-quan": "空明拳共七十二路，口訣是：空朦洞松、風通容夢、沖窮中弄、童庸弓蟲。",
...
int ask_story2()
{
        ...
        if (me->query_temp("thd/story") == 1){
                ...
                msg+= HIW"周伯通說完故事，又無聊的問道：“兄弟，有什麼好玩的武功么？\n"NOR;
                message_vision(msg, me);
...
int ask_jiuyin()
{
	...
	message_vision(HIY"周伯通拉着$N說道：“乖乖的聽我說故事吧。”\n"NOR,ob);
	command("grin");
	...
	if (hubo == 22) {
          if( (ob->query_temp("sj_credit/quest/public/hubo")&&random(ob->query("pur"))>18) 
				...
				message_vision("$N微微一笑，凝神守一，心地空明，隨隨便便的伸出雙手手指，左手畫了一個方塊，右手畫了一個圓圈，\n"+
					"方者正方，圓者渾圓。\n", ob);
				say("周伯通大吃一驚，過了半晌才道：“這定是你從娘胎里學來的本領，那便易辦了。”\n");
...
{
	string msg;
	msg = HIC"周伯通說道：“挨打沒關係，多打你就習慣啦，好久沒跟人打架了。”\n";
