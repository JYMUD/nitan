...
int ask_pearl()
{
     object me;
     me = this_player();
     command("say 本周你已經對着它祈禱(pary)過"+ HIR+chinese_number(me->query("spec/pearl")) +NOR"次了。");
...
int ask_taijitu()
{
     object me;
     me = this_player();
     command("say 本周你已經盯着這張太極圖研究(yanjiu taiji tu)"+ HIR+chinese_number(me->query("spec/taijitu")) +NOR"次啦。");
...
int ask_zhanshu()
{
     ...
     command("say 獨孤九劍，難道天下真的有這麼高明的劍法么？ ");
     command("think ");
     return 1;
}
