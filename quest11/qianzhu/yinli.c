...
void create()
{
	...
	set("chat_msg", ({
		(: random_move :),
		CYN"殷離突然自言自語道：“死鬼！我很老了么？老得像你媽了？”，說完又吃吃地笑了起來。\n"NOR,
		(: random_move :),
		"殷離從地下拾起一根柴枝，在地上亂抽了兩下。\n",
		(: random_move :),
		CYN"殷離突然板著臉道：你取笑我生得丑，你不想活了。\n"NOR,
		(: random_move :),
...
string ask_qzs()
{
	...
	if ( quest && quest["pass"]) {
		...
		return "嗯，這裡有個我才修鍊過的金盒，既然你要就拿去吧。";
	}
	else {
		...
		if ( me->query("registered") < 3 && quest && quest["fail"] >= 3 ){
			destruct(ob);
			return "你不適合修鍊千蛛萬毒手，還是不要再打探了吧。";
		}
		if ( quest && quest["fail"] >= i ) {
			destruct(ob);
			return "你短時間內還不適合修鍊千蛛萬毒手，還是過段時間再來打聽吧。";
		}
		...
		if (random(me->query_int(1)) > 30
		 && random(me->query_con(1)) > 35
			...
			ob->move(me);
			return "嗯，你就拿着這個金盒去修鍊(xiulian)千蛛萬毒手吧。";
		}
		else {
			command("look "+ me->query("id"));
			...
			return "看來你還不適合修鍊千蛛萬毒手，過段時間再來問問吧。";
		}
	}
}
