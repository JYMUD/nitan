...
string *parry_msg = ({
        ...
        "$n不退反進，使出恆山劍招"+(order[random(13)])+"「綿里藏針」"NOR"，森森劍氣充溢四周！架開了$P的這招\n",
});
mapping *action = ({
...
([      "action" : "$N舉起$w運使「" CYN "太極劍" NOR "」劍意，划出大大小小無"
                   "數個圓圈，無窮無盡源源不絕地纏向$n",
        "force" : 230,
        "attack": 105,
        "dodge" :-285,
...
int action_po(object me, object victim, object weapon, int damage)
{
        ...
        else if ( damage == RESULT_PARRY ) //對手招架住 可順勢再創對手
        {
                ...
                if( random(skill)>140 )
                {
                        ...
                        message_combatd(HIM"$N劍氣縱橫! 一道劍氣划過了$n的胸口。結果鮮血狂噴!\n"NOR,me,victim);
