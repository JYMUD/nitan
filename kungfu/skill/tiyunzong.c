...
int valid_learn(object me)
{
        if ((int)me->query_skill("dodge", 1) < 80)
                return notify_fail("你基本輕功修為有限，難以修鍊梯雲縱。\n");

        if( query("max_neili", me)<800 )
                return notify_fail("你內力修為太淺，難以修鍊梯雲縱。\n");
        return 1;
}
...
mixed valid_damage(object ob, object me, int damage, object weapon)
{
        ...
        if (ap / 2 + random(ap) < dp)
        {
                ...
                {
                case 0:
                        result += (["msg" : HIC "$n" HIC "一招「白鶴衝天」，身體向上筆直地縱起丈余，"
                                            "$N頓然失去目標，勁招失手！\n" NOR]);
                        break;
                case 1:
        ...
        if (mp >= 100)
        {
                ...
                {
                case 0:
                        result = HIY "$n" HIY "一招「白鶴衝天」，身體向上筆直地縱起丈余，"
                                 "可是$N" HIY "早以看破$n的企圖。\n" NOR;
                        break;
                case 1:
