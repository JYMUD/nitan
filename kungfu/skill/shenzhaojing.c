...
([      "action" : "$N退後一步，雙掌回收，凌空划出一個圓圈，頓時一股澎湃的氣勁直涌$n而出",
        "dodge"  : 75,
        "force"  : 410,
        "attack" : 163,
        "parry"  : 67,
...
{
        if( query("str", me)<32 )
                return notify_fail("你先天膂力孱弱，無法修鍊神照經神功。\n");

        if( query("con", me)<32 )
                return notify_fail("你先天根骨孱弱，無法修鍊神照經神功。\n");
