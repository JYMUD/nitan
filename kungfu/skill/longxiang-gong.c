...
int valid_learn(object me)
{
        ...
        if( query("str", me)<30 )
                return notify_fail("你的先天膂力孱弱，無法修鍊龍象般若功。\n");

        if( query("con", me)<30 )
                return notify_fail("你的先天根骨孱弱，無法修鍊龍象般若功。\n");

        if( query("gender", me) == "無性" && layer >= 1 )
                return notify_fail("你無根無性，陰陽不調，無法修鍊龍象般若功。\n");
        ...
        if (me->query_skill("force", 1) < lvl)
                return notify_fail("你的基本內功水平不夠，難以修鍊更深厚的龍象般若功。\n");

        if( query("max_neili", me)<layer*500 )
                return notify_fail("你的內力修為不夠，難以修鍊更深厚的龍象般若功。\n");

        if (me->query_skill("lamaism", 1) < layer * 25)
                return notify_fail("你對密宗心法理解不夠，難以修鍊更深厚的龍象般若功。\n");

        if (me->query_skill("unarmed", 1) < lvl && layer >= 5)
                return notify_fail("你的基本拳腳水平不夠，難以修鍊更深厚的龍象般若功。\n");
...
mixed hit_ob(object me, object victim, int damage_bonus)
{
        ...
        if (layer >= 10 && random(2) == 1)
        {
                ...
                return HIY "$N" HIY "拳鋒之上竟蘊藏着『" HIR "十龍十象"
