...
int perform(object me,object target)
{
        ...
        if( query("no_fight", environment(me)) )
                return notify_fail("這裡太嘈雜，你不能靜下心來修鍊。\n");
        ...
        if( objectp(query_temp("weapon", me)) )
                return notify_fail("你必須空手才能修鍊千蛛萬毒手！\n");

        if( me->is_fighting() )
                return notify_fail("戰鬥中無法修鍊千蛛萬毒手！\n");
        ...
        if( query_temp("wudu_suck", me) )
                return notify_fail("你正在修鍊中！\n");

        if (! me->can_improve_skill("qianzhu-wandushou"))
                return notify_fail("你的實戰經驗不夠，無法繼續修鍊千蛛萬毒手！\n");
