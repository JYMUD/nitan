...
int exert(object me, object target)
{
        ...
        */
        ...
        if( skill < 1000 )
                return notify_fail("你的臨濟十二庄修為還不夠，無法使用鳳凰涅磐！\n");
        ...
        if( objectp(target) && target != me ) {
                ...
                target->clear_weak();
                message_combatd(HIY "只見$N" HIY "屏息靜氣，雙掌貼在"+target->name()+"背後，交錯運行大小二庄，只見"+target->name()+"全身被紫芒籠罩。\n" NOR, me);
...
void remove_effect(object me)
{
        if( query_temp("niepan", me)){
                ...
                tell_object(me, "你的臨濟庄運行完畢，將內力收回丹田。\n");
        }
}
