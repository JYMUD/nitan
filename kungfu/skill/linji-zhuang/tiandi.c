...
int exert(object me, object target)
{
        ...
        if (level < 30) return notify_fail("你的臨濟十二庄修為還不夠。\n");

        if (me->is_fighting())
                return notify_fail("戰鬥中不能運用天地二庄。\n");
        ...
        if( query_temp("linji/tiandi", me) )
                return notify_fail("你已經運用天地二庄聚精氣於身了。\n");

        set_temp("linji/tiandi", 1, me);
        write( HIY "你席地而坐，五心向天，運行天地二庄，益氣昇陽，益陰潛陽，升降反正，天地二氣交泰於身，頓覺自己精氣上限增加了。\n" NOR);
        message_combatd(
