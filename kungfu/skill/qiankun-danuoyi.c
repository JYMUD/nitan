...
int valid_learn(object me)
{
        ...
        if (query("gender", me) == "無性" &&
            me->query_skill("qiankun-danuoyi", 1) >= 50)
                return notify_fail("你試圖按照心法修鍊大挪移，卻"
                                   "覺得心中一陣迷茫，難以繼續。\n");
        ...
        if ((int)query("max_neili", me) < 6000)
                return notify_fail("你的內力修為太差，無法修鍊乾坤大挪移。\n");

        if ((int)me->query_skill("force") < 450)
                return notify_fail("你的內功火候不夠，無法修鍊乾坤大挪移。\n");
...
void skill_improved(object me)
{
        ...
        if (! layer)
        {
                tell_object(me, HIM "你潛心修鍊乾坤大挪"
                            "移心法，若有所悟。\n" NOR);
        } else
        if ((lvl % 50) == 0)
        {
                tell_object(me, HIM "你修鍊成了第" + chinese_number(layer) +
                            "層的乾坤大挪移心法。\n" NOR);
        } else
