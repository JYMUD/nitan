...
([        "action" : "$N運力於掌，使出"HIY"「盪魔除妖」"NOR"，飛身疾進，手中$w橫掃$n的$l",
        "skill_name" : "盪魔除妖",
        "lvl" : 56
]),
});
...
int valid_learn(object me)
{
        if( query("max_neili", me)<800 )
                return notify_fail("你的內力修為不夠，難以修鍊金剛降魔杵。\n");

        if ((int)me->query_skill("force") < 100)
                return notify_fail("你的內功火候太淺，難以修鍊金剛降魔杵。\n");

        if ((int)me->query_skill("hammer", 1) < 50)
                return notify_fail("你的基本錘法不夠，難以修鍊金剛降魔杵。\n");
