...
int valid_learn(object me)
{
        ...
        if( query("dex", me)<30 )
                return notify_fail("你的先天身法孱弱，無法修鍊參合指。\n");

        if ((int)me->query_skill("force", 1) < 200)
                return notify_fail("你的內功火候不夠，無法修鍊合指。\n");

        if( query("max_neili", me)<4500 )
                return notify_fail("你的內力太弱，無法修鍊參合指。\n");

        if ((int)me->query_skill("finger", 1) < 160)
                return notify_fail("你的基本指法火候不夠，無法修鍊參合指。\n");
        ...
        */
        ...
        if (me->query_skill("martial-cognize", 1) < 120)
                return notify_fail("你的武學修養尚待提高，暫且無法修鍊參合指。\n");
