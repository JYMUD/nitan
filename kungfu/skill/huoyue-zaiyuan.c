...
int valid_learn(object me)
{
        ...
        if( query("str", me)<31 )
                return notify_fail("你的先天膂力孱弱，難以修鍊降龍十八掌。\n");

        if( query("con", me)<24 )
                return notify_fail("你的先天根骨孱弱，難以修鍊降龍十八掌。\n");

        if ((int)me->query_skill("force") < 300)
                return notify_fail("你的內功火候不夠，難以修鍊降龍十八掌。\n");

        if( query("max_neili", me)<3000 )
                return notify_fail("你的內力修為不足，難以修鍊降龍十八掌。\n");

        if ((int)me->query_skill("strike", 1) < 180)
                return notify_fail("你的基本掌法火候不夠，難以修鍊降龍十八掌。\n");
