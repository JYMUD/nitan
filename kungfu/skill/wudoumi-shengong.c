...
([      "action" : "$N一聲呼嘯，雙掌回收，凌空划出一個圓圈，頓時一股熱浪直涌$n而出",
        "dodge"  : 41,
        "force"  : 451,
        "attack" : 113,
        "parry"  : 47,
...
/*
        因考慮五斗米神功的反噬情況，無玄門正宗的內功不能修鍊高，所以暫
...
*/
        ...
        if (me->query_skill("force", 1) < me->query_skill("wudoumi-shengong", 1))
                return notify_fail("你的基本內功水平不夠，難以鍛煉更深厚的五斗米神功。\n");

        if( query("can_perform/wudoumi-shengong/gui", me )
           && me->query_skill("unarmed", 1) < me->query_skill("wudoumi-shengong", 1))
                return notify_fail("你的基本拳腳水平不夠，難以鍛煉更深厚的五斗米神功。\n");
