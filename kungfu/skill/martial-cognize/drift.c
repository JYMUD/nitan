...
int perform(object me, object target)
{
        ...
        if( query("trds", environment(me)) )
                return notify_fail("屠人場里無法使用天龍之翼！\n");     
        ...
        for (i = 0; i < sizeof(inv); i++)
        {
                ...
                if (inv[i]->is_muxiang())
                        return notify_fail("你又不是飛機，背着這麼大一個木箱，能飛得動么？\n");
        }
...
void select_target(object me, string name)
{
        ...
        /* Sigh, 失敗。鑒於目前華夏已經是一個運行“穩定”地MUD，就算是設計上的失敗，也已經難以
        從本質上作出有效的控制，多次討論並參考一些玩家意見後對天龍之翼地控制嘗試迫於阻力取消，
        原因很大程度上源於玩家的習慣性思維方式很難改變。至此宣布所有有關解體的設想與構思也一併
        取消，本巫師不再對此作任何嘗試。
