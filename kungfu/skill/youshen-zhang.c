...
string *dodge_msg = ({
        ...
        "$n游身穿梭，單掌飄忽不定，繽紛拍出，將$N的攻勢全部擋回，只能自保。\n",
...
int valid_learn(object me)
{
        if( query_temp("weapon", me) || query_temp("secondary_weapon", me) )
                return notify_fail("學習游身八卦掌必須空手。\n");

        if ((int)me->query_skill("force") < 100)
                return notify_fail("你的內功火候不夠，無法學游身八卦掌。\n");

        if ((int)me->query_skill("dodge", 1) < 80)
                return notify_fail("你的基本輕功火候不夠，無法學游身八卦掌。\n");

        if ((int)me->query_skill("unarmed", 1) < 80)
                return notify_fail("你的基本拳腳火候不夠，無法學游身八卦掌。\n");

        if( query("max_neili", me)<800 )
                return notify_fail("你的內力太弱，無法練游身八卦掌。\n");

        if ((int)me->query_skill("unarmed", 1) < (int)me->query_skill("youshen-zhang", 1))
                return notify_fail("你的基本拳腳水平有限，無法領會更高深的游身八卦掌。\n");

        if ((int)me->query_skill("dodge", 1) < (int)me->query_skill("youshen-zhang", 1))
                return notify_fail("你的基本輕功水平有限，無法領會更高深的游身八卦掌。\n");
...
int practice_skill(object me)
{
        ...
        if( query("neili", me)<80 )
               return notify_fail("你的內力不夠游身八卦掌。\n");
