...
([      "action": "$N雙手齊劃，跟着雙掌齊推，一招「衝天掌蘇秦背劍」，一股排山般掌力直撲$n",
        "force" : 267,
        "dodge" : 28,
        "parry" : 30,
        "attack": 36,
...
int valid_learn(object me)
{
        if( query_temp("weapon", me) || query_temp("secondary_weapon", me) )
                return notify_fail("練白鶴衝天掌必須空手。\n");

        if ((int)me->query_skill("force") < 100)
                return notify_fail("你的內功火候不夠，無法學白鶴衝天掌。\n");

        if( query("max_neili", me)<800 )
                return notify_fail("你的內力太弱，無法練白鶴衝天掌。\n");
        ...
        if ((int)me->query_skill("strike", 1) < (int)me->query_skill("chongtian-zhang", 1))
                return notify_fail("你的基本掌法水平有限，無法領會更高深的白鶴衝天掌。\n");
...
int practice_skill(object me)
{
        ...
        if( query("neili", me)<100 )
                return notify_fail("你的內力不夠練白鶴衝天掌。\n");
