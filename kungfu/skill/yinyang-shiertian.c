...
int valid_learn(object me)
{
        ...
        if (me->query_skill("martial-cognize", 1) < level)
                                return notify_fail("你武學修養不夠，無法繼續修鍊。\n");
        
        if( level>380 && query("yinyang-shiertian/yinyang", me) )
                                return notify_fail("你陰陽九轉十二重天已達到新的層次，須修鍊出陰陽境界方能再次提升。\n");
        
        if( level>480 && query("yinyang-shiertian/qiankun", me) )
                                return notify_fail("你陰陽九轉十二重天已達到新的層次，須修鍊出乾坤境界方能再次提升。\n");

        if( level>600 && query("yinyang-shiertian/shier", me) )
                                return notify_fail("你陰陽九轉十二重天已達到新的層次，須修鍊出十二重天境界方能再次提升。\n");      
