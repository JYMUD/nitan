...
([        "skill_name" : "靈蛇游八方",
        "action" : "$N使出"HIY"「靈蛇游八方」"NOR"，身形散作八處同時向$n的$l出掌攻擊",
        "lvl"    : 90
]),
([        "skill_name" : "金蛇探頭",
        "action" : "$N使出金蛇游身掌法"HIY"「金蛇探頭」"NOR"，如鬼魅般欺至$n身前，一掌拍向$n的$l",
        "lvl"    : 100
]),
([        "skill_name" : "金龍升天",
...
int valid_learn(object me)
{
        if( query("max_neili", me)<500 )
                return notify_fail("你的內力不足，無法練金蛇游身掌。\n");

        if ((int)me->query_skill("force") < 80)
                return notify_fail("你的內功火候不夠，無法練金蛇游身掌。\n");

        if ((int)me->query_skill("strike", 1) < (int)me->query_skill("jinshe-zhang", 1))
                return notify_fail("你的基本掌法水平有限，無法領會更高深的金蛇游身掌。\n");
...
string *parry_msg = ({
        "$n使出「粘」字訣，雙掌一划，引偏了$N的$w。\n",
});

string *unarmed_parry_msg = ({
        "$n施展出金蛇游身掌，輕描淡寫的化解了$N的攻勢。\n",
});
