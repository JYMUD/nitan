...
([
"action":"$N大喝一聲，一招「飛龍在天」，$w脫手飛出，夾着勁風射向$n的前心，隨即搶到$n的身後，伸手又把$w抄在手中",
         "force" : 300,
         "dodge" : 20,
         "parry" : 30,
        "damage": 80,
...
"$N雙手交錯，持槍划了兩個圓圈，罡氣護住全身，竟無半分空隙，將$n之來勢化解與無形。\n",
...
HIC"$N凝望$n招式，出槍以蜿蜒之勢、身處庄穩之形，宛如旭日東升，如金蛇萬道，閃爍不定"NOR,
HIB"$N槍法突變，招式凌厲狠辣，以極渾厚內力，出極精妙招數，青光蕩漾"NOR,
...
HIR"$N左手$w"HIR"緩緩向前划出，成一弧形。$n只覺一股森森寒氣，直逼過來"NOR,
...
mixed hit_ob(object me, object victim, int damage_bonus, int factor)
{
    	...
    	if(weapon1 && me->query("jiali")
       	...
       	&& random(me->query("combat_exp")) > victim->query("combat_exp")/4){
      		...
      		if(random(me->query("str")) > victim->query("str")/3){           
           		...
           		message_vision(HIY"$N一招使出，真力運得不足，被"+weapon->name()+HIY"帶着連轉幾個圈子，手中"+weapon1->name()+HIY"一震，險些脫手！\n"NOR, victim);
           		victim->add_busy(3);
           	}
    	}
   ...
   && random(me->query_str()+me->query_con()) > random(victim->query_int()*2) 
	&& random(10)>6){
    						...
    						message_vision(HIC"$P越斗越害怕，被$p帶得招式漸見澀滯！\n"NOR, victim, me);
    						i *= 2;
    					}
