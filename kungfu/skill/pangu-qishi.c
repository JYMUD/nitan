...
([      "action" : "$N將手中$w划出一道半弧，一式「斷岳」便如流星墜地，直轟$n",
        "skill_name" : "斷岳",
        "force" : 460,
        "attack": 38,
        "dodge" : -27,
...
{
        if( query("str", me)<32 )
                return notify_fail("你的先天膂力孱弱，無法修鍊盤古七勢。\n");

        if( query("max_neili", me)<2000 )
                return notify_fail("你的內力修為太淺，無法修鍊盤古七勢。\n");

        if ((int)me->query_skill("force") < 180)
                return notify_fail("你的內功火候太淺，無法修鍊盤古七勢。\n");

        if ((int)me->query_skill("hammer", 1) < 80)
                return notify_fail("你的基本錘法火候太淺，無法修鍊盤古七勢。\n");
