...
([      "action" : "$N默念口訣運氣經脈，只覺愈轉愈是順遂，當下一個翻身躍起，咕的一聲叫喊，雙掌對着$n拍出",
        "lvl" : 110,
        "skill_name" : "蛤蟆功"
]),
...
([      "action":"$N驀地張口，白牙一閃，已向$n$l咬落，又快又准",
        "dodge": -5,
        "force": 420,
        "damage": 80,
        "weapon": "牙齒",
...
int valid_learn(object me)
{
        ...
        /*
        if( query("con", me)<32 )
                return notify_fail("你先天根骨孱弱，無法修鍊蛤蟆功。\n");

        if( query("dex", me)<32 )
                return notify_fail("你先天身法太差，無法修鍊蛤蟆功。\n");
        */
        ...
        */
        ...
        if(me->query_skill("hamagong", 1) > 99){
                ...
                if ( k >=3 )
                return notify_fail("你體內不同內力互相衝撞，難以領會更高深的內功。\n");
        }
