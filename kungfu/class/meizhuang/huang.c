...
void attempt_apprentice(object me)
{
        ...
        command("say 不錯，不錯！");
        command("say 我就收下你了，不過千萬別招惹是非，給我梅庄添亂。");
        command("recruit "+query("id", me));

        if( query("move_party/日月神教―梅庄", me) )
        {
                delete("move_party/日月神教―梅庄", me);
                command("say 現在你成為我梅庄中人，望你專心練武，別再去過問教中事物。");
        }
}
...
mixed ask_skill1()
{
        ...
        if( query("family/gongji", me)<500 )
                return "你對本庄所作出的貢獻不夠，這一招我暫時還不能傳你。";
...
mixed ask_xiao()
{
        ...
        if( query("family/family_name", me) != "梅庄" )
                return "你和本庄素無淵源，打聽它幹什麼？";
        ...
        if (objectp(owner) && owner != this_object())
        {
                ...
                if( query("family/family_name", owner) == "梅庄" )
