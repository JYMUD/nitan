...
string* qin_msg1 = ({
        ...
        "我這裡的花園養着很多的動物，就是沒有name，",
        "在空閑的時候，看看動物的生活能夠放鬆情緒，name其實挺可愛的，",
...
int accept_fight(object me)
{
        ...
        if( query("qi", me) <= 0 )
        {
                message_vision("\n$N搖搖頭對$n道：“你現在的武功"
                               "還差得遠，連我一招都接不住！”\n",
                               this_object(), me);
                return notify_fail("看來要好好的修鍊功夫了。\n");
        }
...
int give_quest()
{
    ...
    */
    ...
    {
        write("南海神尼呵呵笑着說：心急可是喝不了熱粥啊，不要着急，慢慢來。\n");
        return 1;
    }

    if( mapp(query("quest_kh", me)) )
    {
        write("南海神尼微微一怔說：葵花太監不是給了你任務了么？要不你還是先完成手頭的任務吧。\n");
        return 1;
    }
    ...
    {
        write("南海神尼笑着對你說道：“怎麼完不成任務這麼不順利？不要着急，先好好想想原因在哪裡。\n");
        return 1;
    }
...
/*
    if (quest["type"] == "殺" && (random(6) == 3 || wizardp(me)))
    {
        ...
        msg = "南海神尼微笑着說道：“我剛將武林同盟的口令更改了一下，你去通知一下" + quest["name"] + NOR"，新口令是：\n";
...
int accept_object(object me, object ob)
{
    ...
    if ( quest["type"] == "送" )
    {
        ...
        {
            write("南海神尼接過來一看，皺眉道：“這張回執是你自己得到的么？別想來糊弄我！”\n");
            return 0;
        }
...
//    message_vision("$N將$n交給南海神尼。\n",me,ob);與give信息重複haiyan
    ...
    write("南海神尼接過來開心的笑着說：“不錯呀！你這次任務完成得很好！”\n");
    destruct(ob);
    if(!random(10))
    {
        i=(exp/60+1);
       ...
       if (total_count == 100)
   {
    ...
    if (total_count == 450)
       ...
       if (total_count == 0)
   {
       msg = "$N深深嘆了口氣，道：“沒想到你連着"+chinese_number(quest_count)+
...
int give_up()
{
    ...
    */
    if( (quest=query("quest_sn", me)) )
    {
        ...
        if (quest["type"] == "傳")
        {
                message_vision("$N皺着眉頭對$n說：『不就是傳一個口令么？有那麼難？回去仔細想想，這個任務一定要完成！\n",this_object(),me);
                return 1;
        }
        */
        if (quest["type"] == "送")
        ...
        message_vision("南海神尼對$N微笑着說:“沒關係，下次好好努力就行啦。”\n", me);
