...
/*
    if (questkh["type"] == "殺" && (random(6) == 3 || wizardp(ob)))
    {
        ...
        msg = "葵花太監陰笑着說道：“我剛將出入宮門的口令更改了一下，你去通知一下" + questkh["name"] + NOR"，新口令是：\n";
...
int accept_object(object who, object ob)
{
   ...
   switch(questkh["type"])
   {
      case "尋"  :
         ...
         {
            tell_object(who,"葵花太監大怒，斥道：“這是什麼東西？你想來糊弄我么！”\n");
...
//   message_vision("$N將$n交給葵花太監。\n",who,ob);與give信息重複haiyan
       ...
       if (total_count == 100)
   {
    ...
    if (total_count == 450)
       ...
       if (total_count == 0)
   {
       msg = "$N深深嘆了口氣，道：“想不到你連着"+chinese_number(quest_count)+
...
int give_up()
{
   ...
   if( mapp(questkh=query("quest_kh", ob)) )
   {
      ...
      if (questkh["type"] == "傳")
      {
         message_vision("$N勃然大怒，指着$n鼻子罵道：『不就是傳一個口令么？這麼點小事都完成不了？還不快滾！\n",this_object(),ob);
         return 1;
      }
      */
      if(questkh["desc"] == "滅")
...
void npc_dest(object npc)
{
    ...
    if(npc->is_character())
        message_vision("$N哈哈一笑：“葵花太監的手下儘是一些窩囊廢！”轉身一晃，已不見了身影。\n",npc);
