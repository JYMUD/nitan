...
void init()
{
        ...
        {
                say("游坦之大叫一聲：“好你個"+RANK_D->query_rude(this_player())+"，居然想殺了阿紫姑娘，我跟你拼了！！”\n");
        ...
        player=query_temp("last_damage_from", ob);
                ...
                {       
                        ...
                        {   
                                who->kill_ob(player);
                                player->fight_ob(who); 
                                tell_object(player,"游坦之大怒：“狗賊，你把阿紫姑娘傷成這樣，你去死吧！”\n");
                        }
                        else 
                        {
                                who->set_leader(player);
                                tell_object(player,"游坦之大聲叫道：“你要把阿紫姑娘帶到哪裡去？快把阿紫姑娘還給我！除非殺(kill)了我，要不然我絕對不會允許你把阿紫姑娘帶走！”\n");
...
void die()
{
        ...
        if ( objectp( player ) ) 
        {
        message("vision","游坦之面目猙獰之極，手突然揚起，滿是鮮血的
臉上居然露出了一絲詭異的笑容，這下用盡了他
所有的力氣，終於緩緩的倒了下去。\n",environment(player));
        message("vision","游坦之大叫一聲，死了！\n",environment(player));
        tell_object(player, 
        "你覺得臉上似乎沾上了什麽東西，伸手一摸卻什麽也沒有。\n");
        player->apply_condition("bingcan_poison", 40);
        tell_object(player,"你搜了搜游坦之的屍體，發現胸口處有件東西，你連忙把它掏了出來。\n");
...
int accept_kill(object me)
{
        ...
        who=find_living("a zi");
                if(who)
                {
                        message("vision","游坦之抓住阿紫的胳膊，一縱身飛也似的去了。\n",environment(me));
...
int accept_fight(object me)
{
        if( !query_temp("azi/killer", me) || !query_temp("azi/killer2", me) )
        {
        tell_object(me,"游坦之說道：“我可沒心思和你玩！”\n");
        return 0;
        }
        else
        {
        tell_object(me,"游坦之怒道：“既然你這麼急着投胎，我就成全你！”\n");
        kill_ob(me);
        return 1;
        }
        return 1;
