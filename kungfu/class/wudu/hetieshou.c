...
mixed ask_she()
{
        ...
        command("whisper"+query("id", me)+"這含沙射影里的毒針用完了，可以拿"
                "(show)回來讓我給你裝。");
        return 1;
}
...
int do_show(string arg)
{
        ...
        if( query("zhen", ob)>5 )
                return notify_fail(CYN "何鐵手說道：含沙射影里的毒針用了一半"
                                   "還不到，你瞎急什麼！\n" NOR);

        message_vision(HIC "\n何鐵手微微一笑，從$N" HIC "手中接過含沙射影，輕"
                       "輕撥弄幾下，又還給了$N" HIC "。\n\n" NOR, me);

        command("say 行了，含沙射影里的毒針已經補滿了。");
        set("zhen", 10, ob);
        return 1;
}
