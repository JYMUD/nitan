...
int accept_kill(object ob)
{
        ...
        {
                command("say 好傢夥，竟想謀害老夫！師弟，準備雙刀合壁！");
                obj->kill_ob(ob);
        }
...
int ask_why(object obj, object ob)
{
        ...
        message_vision("$N皺着眉頭對$n說道：咦！你不是已閉關靜修過了嗎？\n", me, ob);
        command("say 紫霞遺頁交給我保管也好，你還有甚麽事嗎？");
...
int thank_huashan(object obj, object ob)
{
        ...
        if( query("id", me) == "gaolao" && ob1=present("ailao",environment(me)) )
        {
                ...
                message_vision("$N將矮老者拉到一旁細說了一番，只見矮老者臉色變了幾變，兩人低頭沉思着。\n", me);
                me = ob1;
        }
        else if( query("id", me) == "ailao" && present("gaolao",environment(me)) )
        {
                command("say 讓我和師弟商量商量，你便在此等着吧。");
                message_vision("$N將高老者拉到一旁細說了一番。高老者聽罷臉色變了幾變，兩人低頭沉思着。\n", me);
        }
        else
        {
                command("say 讓我仔細研究此箋，你便在此等着吧。");
                message_vision("$N說完後便獨自走到一旁坐下，全神貫注地研讀起着紫霞遺頁來。\n", me);
        }
...
int open_room(object me, object ob, object obj)
{
        ...
        {
                command("say 好！我便為你護法。你安心在密室里閉關修練玄功吧。");
                message_vision("$N將紫霞遺頁交還給$n。\n", me, ob);
                message_vision("接着$N轉身向牆上一處一按一掀，牆壁正中轟隆隆響了一陣，露出一道門來。\n", me);
                command("say 唉！數十年來我始終鑽研不透密室里的用處，原來還有這個秘密。");
                command("say 好好利用密室里的東西，小心別把練功的步驟搞錯了！");
                obj->move(ob);
