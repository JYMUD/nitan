...
string ask_sword()
{
        ...
        if ((int)ob->query_skill("sword", 1) < 200)
                return "這位" + RANK_D->query_respect(ob) + "的基本劍法還需繼續修鍊，否則難以抵擋洪水的衝擊。";
...
mixed teach_sad()
{
        ...
        if( query("gender", ob) != "男性" )
        {
                command("say 我這門武功只適合男性修鍊，你還是算了吧！");
                return 1;
        }
...
int accept_object(object me, object ob)
{
        ...
        {
                tell_object(me, HIG "楊過告訴你：你基本內功修為不足，無法修鍊怒海狂濤。這丹我要不得。\n" NOR);
                return 0;
        }               
        if( query("max_neili", me)<35000 )
        {
                tell_object(me, HIG "楊過告訴你：你內力修為不足，無法修鍊怒海狂濤。這丹我要不得。\n" NOR);
                return 0;
        }       
        if (me->query_skill("taoism", 1) < 1)
        {
                tell_object(me, HIG "楊過告訴你：你道學心法修為不足，無法修鍊怒海狂濤。這丹我要不得。\n" NOR);  
                return 0;
        }               
        if (me->query_skill("xuantie-jian", 1) < 1)
        {
                tell_object(me, HIG "楊過告訴你：你玄鐵劍法修為不足，無法修鍊怒海狂濤。這丹我要不得。\n" NOR);  
                return 0;                               
        }               
        if (me->query_skill("martial-cognize", 1) < 1200)
        {
                tell_object(me, HIG "楊過告訴你：你武學修養不足，無法修鍊怒海狂濤。這丹我要不得。\n" NOR);              
                return 0;
        }               
        if (me->query_skill("jiuyin-shengong", 1) < 1 && 
            me->query_skill("jiuyin-hamagong", 1))
        {
                tell_object(me, HIG "楊過告訴你：你未曾練成過九陰真經，無法修鍊怒海狂濤。這丹我要不得。\n" NOR);
                return 0;       
        }               
        ...
        tell_object(me, HIG "楊過將怒海狂濤絕學修鍊要決一一傳授於你...\n" NOR);
