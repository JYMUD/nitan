...
void create()
{
        ...
        set("long",
                "這是一輛非常結實的鏢車，由兩匹大騾子拉着，不知裝着什麼東西。鏢車上豎著一面大旗，上書“福威鏢局”四個大字。\n");
        set("unit", "輛");
        set_weight(10000);
                ...
                for(i=0;i<sizeof(obj);i++)
                {
...

                        {
...

                                {
                                        ...
                                        message_vision(HIY "只見$N應聲衝著$n撲了過去！喝道："+RANK_D->query_rude(target)+HIY"拿命來吧！\n" NOR, obj[i], target);
...
int do_check()
{
        ...
        else if( query_temp("biao/dest", obj) == "yeerniang")write("這是西夏葉二娘托送的重鏢，由"+query("owner", obj)+"壓運，要儘快送到。\n");
        else if( query_temp("biao/dest", obj) == "fengyiming")write("這是佛山英雄會館鳳一鳴的重鏢，由"+query("owner", obj)+"壓運。\n");
        else if( query_temp("biao/dest", obj) == "wangtongzhi")write("這是泉州濟世堂老店的重鏢，由"+query("owner", obj)+"壓運，務必交到藥鋪掌柜王通治手裡。\n");
