...
int do_name(string arg)
{
        ...
        if (arg == name())
                return notify_fail("人家現在不就是叫這個名字么？\n");
...
{
        if (! arg)
                return notify_fail("你要給" + name() + "一個什麼昵稱？\n");
...
mixed accept_ask(object who, string topic)
{
        ...
        if (topic == "name" || topic == "名字" || topic == "姓名")
        {
                ...
                return "我的名字叫" + name() + "呀，你不記得了么？";
        }

        if (topic == "nick" || topic == "昵稱" || topic == "綽號")
        {
                // 詢問昵稱
                nick = query("nickname");
                if (who != owner)
                ...
                return "我現在叫" + nick + NOR CYN "呀，你打算給我換(snick)一個么？";
        }
...
void relay_emote(object me, string arg)
{
        ...
        case "tnnd":    command("say 你這人好噁心哦。");break;
...
string description()
{
        ...
        per = query_per();
        if (per >= 30) return "她有傾國傾城之貌，容色麗郁，嬌艷絕倫，堪稱人間仙子！\n";
