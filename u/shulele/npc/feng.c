	...
	// 已經有人開始舉牌
        ...
        case 1:
                message_vision(CYN "$N" CYN "皺了皺眉頭，說道：有人願意購買"
                               "么？請即時舉牌(" HIY "jupai" NOR + CYN ")。\n"
                               NOR, operator);
                break;

        case 2:
                message_vision(CYN "$N" CYN "微笑道：各位考慮好了么？有人願"
                               "意購舉牌(" HIY "jupai" NOR + CYN ")出價么？\n"
                               NOR, operator);
                break;
...
int do_qiao()
{
        ...
        if (m = query("on_jupai"))
        {
                ...
                return notify_fail("您準備繼續宣布拍賣哪個店鋪？\n");
