...
string *body_damage_me_msg = ({
        append_color(HIR "你只覺$l隱隱作痛，體內真氣受到一點振蕩。\n" NOR, HIR),
...
string damage_msg(int damage, string type)
{
        ...
        case "割傷":
                ...
                if( damage < 200 ) return "結果在$p$l划出一道細長的血痕。\n"; else
                if( damage < 400 ) return "結果「嗤」地一聲，$w已在$p$l划出一道傷口！\n"; else
                if( damage < 600 ) return "結果「嗤」地一聲，$w已在$p$l划出一道血淋淋的傷口！\n"; else
                if( damage < 1000 )return "結果「嗤」地一聲，$w已在$p$l划出一道又長又深的傷口，濺得$N滿臉鮮血！\n"; else
                                   return "結果只聽見$n一聲慘嚎，$w已在$p$l划出一道深及見骨的可怕傷口！\n";
                break;
        case "刺傷":
        ...
        case "點穴":
                ...
                if( damage < 1000 )return "結果$n一聲大叫，$l的穴道被點中,疼痛直入心肺！\n"; else
                                   return "結果只聽見$n一聲慘叫，一陣劇痛夾雜着麻癢游遍全身，跟着直挺挺的倒了下去！\n";
                break;
        case "抽傷":
        case "鞭傷":
...
varargs string eff_status_msg(int ratio, int dam_type) 
{
        ...
        if( ratio > 30 ) return HIR "傷痕纍纍，正在勉力支撐著不倒下去。" NOR;
...
int player_escape(object killer, object ob)
{
        ...
        */
        ...
        tell_object(ob, "你睜開眼來，看到兩位老者正在靜坐修鍊。\n"
        ...
        if( query("bunch_quest", killer) &&
            ...
            query("bunch_quest", killer)["ob"] == victim){
                ...
                b = ([ "exp" : exp,
                       ...
                       "score" : score,
                       "weiwang" : weiwang,
                       "prompt": "在剷除外來敵人" + victim->name() + HIG "的過程中，經過鍛煉"
                ]);
