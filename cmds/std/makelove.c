...
string *loving_msg = ({ 
        ...
        HIM "洶湧澎湃的高潮過後，$n溫順的躺在$N的懷裡，聽着$N的心跳聲，$N輕輕的撫弄着$n光滑的皮膚，無限溫柔...\n" NOR,
});
...
int main(object me, string arg)
{
        ...
        if( query("gender", me) == "無性" )
                return notify_fail("你這人都這模樣了還有興緻？真是少見。\n");

        if( query("gender", me) == query("gender", target) )
                return notify_fail("同性戀么...還是自己好好研究怎麼弄吧，這條指令不太合適。\n");
        ...
        if( query_temp("pending/makelove", target) == me )
        {
                ...
                if( query("gender", me) == "男性" )
                        ...
                        message_sort(YEL "$N極力的忍受着$n火熱的挑逗，心頭小鹿撞擊不停，終於癱軟在$n的懷裡，"
                                     "被$n一把橫抱了起來，輕柔的放在床上....\n" NOR, me, target);
...
void do_over(object me, object target)
{
        ...
        &&  random(100) < 21)
        {
                message("shout",HIR"【家有喜事】："HIM"恭喜"HIY+query("name", me)+"("+query("id", me)+")"
                        HIM"與"HIY+query("name", target)+"("+query("id", target)+")"HIM"做了准父母。\n"NOR,
                        users());
