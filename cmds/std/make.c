...
int main(object me, string arg)
{
        ...
        if (me->is_fighting())
                return notify_fail("打架的時候你還有閑工夫配藥？\n");
        ...
        if (! qm)
                return notify_fail("你的先把能夠磨葯的研缽拿(hand)在手上才行。\n");
...
int making(object me)
{
        ...
        default:
                step = 0;
                msg_me = "你拿起棒杵，慢慢的搗着" + qm->name() + "裡面的藥材。\n";
        ...
        case 6:
                ...
                msg_out = random(2) ? "$N像老和尚一樣閉目冥神坐在那裡。\n"
                                    : "$N手裡握着" + qm->name() + "沒完沒了的轉來轉去。\n";

                break;
        case 7:
                msg_me = "你運用內力，隔着" + qm->name() + "使裡面的藥材徹底的融合。\n";
                msg_out = "$N眉毛動了動，接着又像老和尚一樣閉目冥神坐在那裡。\n";
                break;
        case 8:
