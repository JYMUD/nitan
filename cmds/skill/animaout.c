...
int main(object me, string arg)
{
        ...
        if( query("reborn/times", me) < 1 )
                return notify_fail("你還沒有轉世過，如何能領悟出修鍊元嬰出世？\n");

        if (wizardp(me))
                return notify_fail("巫師也要修鍊元嬰出世？有毛病了？\n");

        if( !query("no_fight", where) )
                return notify_fail("在這裡修鍊元嬰出世？不太安全吧？\n");
        ...
        if( query("potential", me)-query("learned_points", me)<1000 )
                return notify_fail("你的潛能不夠，沒法閉關修行以修鍊元嬰出世。\n");
