...
int main(object me, string arg)
{
        ...
        case "shier":         
                ...
                if( !query("can_perform/yinyang-shiertian/zhen", me) )
                        return notify_fail("你還沒有修鍊成陰陽境界，就想修鍊十二重天境界？\n");
                 
                if( !query("can_perform/yinyang-shiertian/jiu", me) )
                        return notify_fail("你還沒有修鍊成乾坤境界，就想修鍊十二重天境界？\n");
                        
                if (lvl < 1200)
                        return notify_fail("你的陰陽九轉十二重天修為不夠，難以修鍊十二重天境界！\n");

                if( query("max_neili", me)<50000 )
                        return notify_fail("你覺得內力頗有不足，看來目前還難以修鍊十二重天境界。\n");

                if( query("max_jingli", me)<50000 )
                        return notify_fail("你覺得精力頗有不足，看來目前還難以修鍊十二重天境界。\n");

                if( query("max_qi", me)<50000 )
                        return notify_fail("你覺得氣血頗有不足，看來目前還難以修鍊十二重天境界。\n");

                if( query("potential", me)-query("learned_points", me)<500000 )
                        return notify_fail("你的潛能不夠，沒法修鍊。\n");
        ...
        case "qiankun":        
                ...
                if( !query("can_perform/yinyang-shiertian/zhen", me) )
                        return notify_fail("你還沒有修鍊成陰陽境界，就想修鍊乾坤境界？\n");
                 
                if( query("can_perform/yinyang-shiertian/jiu", me) )
                        return notify_fail("你已經修鍊成乾坤境界，不必再重複修鍊！\n");
                                                        
                if (lvl < 800)
                        return notify_fail("你的陰陽九轉十二重天修為不夠，難以修鍊乾坤境界！\n");

                if( query("max_neili", me)<30000 )
                        return notify_fail("你覺得內力頗有不足，看來目前還難以修鍊乾坤境界。\n");

                if( query("max_jingli", me)<30000 )
                        return notify_fail("你覺得精力頗有不足，看來目前還難以修鍊乾坤境界。\n");

                if( query("max_qi", me)<30000 )
                        return notify_fail("你覺得氣血頗有不足，看來目前還難以修鍊乾坤境界。\n");
        ...
        case "yinyang":
                ...
                if( query("can_perform/yinyang-shiertian/zhen", me) )
                        return notify_fail("你已經修鍊成陰陽境界，不必再重複修鍊！\n");
                        
                if (lvl < 500)
                        return notify_fail("你的陰陽九轉十二重天修為不夠，難以修鍊陰陽境界！\n");

                if( query("max_neili", me)<10000 )
                        return notify_fail("你覺得內力頗有不足，看來目前還難以修鍊陰陽境界。\n");

                if( query("max_jingli", me)<10000 )
                        return notify_fail("你覺得精力頗有不足，看來目前還難以修鍊陰陽境界。\n");

                if( query("max_qi", me)<10000 )
                        return notify_fail("你覺得氣血頗有不足，看來目前還難以修鍊陰陽境界。\n");
        ...
        CHANNEL_D->do_channel(this_object(), "rumor",
                              sprintf("%s%s(%s)開始閉關修行，試圖修鍊陰陽九轉十二重天的" + msg + "境界。",
                              ultrap(me) ? "大宗師" : "",
...
int mending(object me)
{
        ...
        case "shier":
                ...
                {
                        tell_object(me, "你的狀態不佳，無法繼續修鍊陰陽九轉十二重天。\n");
                        message_vision("$N睜開雙目，緩緩吐了一口氣，站了起來。\n", me);
                        msg = "退出";
                }
        ...
        if (msg == "退出")
        {
                CLOSE_D->user_opened(me);
                CHANNEL_D->do_channel(this_object(), "rumor",
                        sprintf("聽說%s(%s)閉關修鍊陰陽九轉十二重天結束，似乎沒有太大的進展。",
                                me->name(1),query("id", me)));
        ...
        if( query("yinyang-shiertian/xiulian", me) == "shier" )
        {
                ...
                default:
                        msg = "$N的嘴角微微上翹，蕩漾出一陣笑意在臉頰上，周圍的白霧隨着笑意"
        ...
        if( query("yinyang-shiertian/xiulian", me) == "qiankun" )
        {
                ...
                case 0:
                        msg =  "$N想了想，說了一聲“山”，然後便一動不動地佇立於天地間，"
                               "整個人從內到外都處於一種極靜止的狀態，任世間怎樣的動蕩，也決不動搖半分。\n";
                        break;
                ...
                default:
                        msg = "$N突然伸出手在半空中划了一道痕迹，那道痕迹彷彿山那般極靜，"
