...
int main(object me, string arg)
{
        ...
        if (n < 1)
                return notify_fail("你要讓別人發獃多久？\n"); 

        if( time()-query_temp("last/idle", me)<10 )
                return notify_fail("如果你要連續使用發獃神功，請使用idle <玩家> <次數>\n");

        if (me == ob)
                return notify_fail("你大腦有水？\n");

        if (wizardp(ob))
                return notify_fail("你的發獃神功等級還沒有那麼高。\n");

        if( query("env/no_idle", ob) )
                return notify_fail("你不能讓這個人發獃...\n");

        lvl = me->query_skill("idle-force", 1);
        if (! lvl)
                return notify_fail("你的先學好了發獃神功再說。\n");

        if (lvl < 50)
                return notify_fail("你的發獃神功水平太差，無法施展絕技。\n");

        if (lvl < 300 && environment(ob) != environment(me))
                return notify_fail("你的發獃神功水平有限，無法施展奇術於千里之外！\n");
        ...
        if (! ob->ban_say(1))
                CHANNEL_D->do_channel(this_object(), "rumor", "聽說" +
                                      me->name(1) + HIM "施展發獃神功，" +
                                      ob->name(1) + HIM "無辜受害。");
        ob->ban_say_until(lvl, "你被嚇傻了，現在說不上話來");
        write(HIC "你的發獃神功成功了，" + ob->name(1) +
              HIC "現在被嚇得說不上話來了！\n" NOR);
...
這個指令可以讓你使某一個玩家不能說話，當你的發獃神功到了
三百級以後可以使不在你面前的玩家閉嘴。運用發獃神功的成功
率與自己的發獃神功等級和對方的經驗有關，每使用一次發獃神
功都會耗費一點潛能和一些精。
