...
{
        if(objectp(present(obj,environment(me)))){
      message_vision(HIC"\n$n對着$N大喝一聲:"+RANK_D->query_rude(me)+"，想走? 沒門！嘗嘗本"+RANK_D->query_respect(obj)+"厲害再說！\n"NOR,me, obj);   
...
void do_puke(object me, object obj)
{
     if(objectp(present(obj, environment(me)))) {
        message_vision(CYN"\n$N稀里哇啦地吐了一地。\n"NOR, me,obj);
        tell_object(me,HIY"你不由得對自己說：天啦，這世上還有這麼丑的女人，我還是站遠些好！\n"NOR);    
        message_vision("\n$N遠遠地推開$n,苦笑着說："+RANK_D->query_respect(obj)+"，如此‘尊容’，在下實在無福消受\n剛才唐突，還望見諒，咋們後會無期！\n",me,obj); 
        message_vision(CYN"\n接着$N又是一陣狂吐。\n"NOR,me,obj);
        me->start_busy(1);
        obj->start_busy(1);
}
...
void do_qiangjian1(object me,object obj)  
{
        ...
        if(objectp(present(obj, environment(me)))) {
        message_vision("接着，$N的大手不顧$n的掙扎在$n的身上來回上下遊走.......\n", me, obj);   
...
void do_qiangjian10(object me,object obj)  
{
        ...
        if(objectp(present(obj, environment(me)))) {
        message_vision(HIM"\n良久，風停雨歇。。。。\n"NOR, me, obj);
        message_vision("$N爬了起來，對着昏睡在地的$n失望地嘆了一下氣：真他媽的倒霉,讓老子遇到這種'水貨'。\n", me, obj);
}
        return;
}
