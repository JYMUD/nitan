...
int do_liaowang( string arg )
{
        ...
        if ( !arg || sscanf( arg, "%d %d", x, y ) != 2 ) return notify_fail("請輸入正確坐標！x -> 豎著數第幾橫  y -> 橫着數第幾個\n");

        if ( x < 1 || x > 10 || y < 1 || y > 16 ) return notify_fail("請輸入正確坐標！x -> 豎著數第幾橫  y -> 橫着數第幾個\n");
...
void target_check_hp()
{
        ...
        if ( target_hp < 1 ) {
                ...
                if ( query("who_name") == "吉野號" ) {
                        ...
                        msg="【閑聊】北洋水師(beiyangshuishi)："+query("name", me)+HIC"成功地擊沉了日本海軍旗艦──吉野號。\n";
                ...
                if ( query("sunk_enemy") > 10 ) {
                        ...
                        addn("job/jiawu", 1, me);
                        tell_object(me, "你被獎勵了" + exp + "點經驗值，" + pot + "點潛能。\n");
                        msg="【閑聊】北洋水師(beiyangshuishi)："+query("name", me)+HIC"成功地擊沉了日本海軍旗艦的攻擊。\n";
                        message("channel:chat", HIC + msg + NOR, users());
                        remove_call_out("del_me");
                        call_out("del_me", 1);
                        return;
