...
int do_withdraw(string arg)
{
        ...
        message_vision(sprintf("$N從銀號里取出%s%s%s。\n",
                chinese_number(amount), query("base_unit", what_ob),
                query("name", what_ob)), me);
...
int do_transfer(string arg)
{
        ...
        ob->save();
        tell_object(ob, "\a", 0);
        tell_object(ob, HIR + me->query_idname(1) + HIR "從銀號里劃轉"+ MONEY_D->money_str(value) + "到你的帳戶上。\n" NOR);
        tell_object(me, HIR "你從銀號里劃轉"+ MONEY_D->money_str(value) + "到" + ob->query_idname(1) + HIR "的帳戶上。\n" NOR);
