...
int do_join()
{
       ...
       if( time()-query("last_leave_bunch", me)<120 )
              return notify_fail(obj->name() + "看着你說：聽說你是剛脫離了別的幫會過來的吧？我得再觀察觀察你，過兩分鐘再來吧！\n");
...
int do_disengage()
{
       ...
       {
               message_vision("$N對着$n嘆氣道：早就知道你不會在這種小地方混一輩子的，也罷，你自己保重！\n",obj,me);
...
int do_choose(string bh)
{
       ...
       if( query("bunch_quest", me) )
               return notify_fail(obj->name() + "生氣的看着你說：你剛才領的任務還沒有完成，急什麼？\n");
       ...
       if (bunch_quest["id"] == "xc" && !bunch_quest["enable"])
               return notify_fail(obj->name() + "微微笑着說：今天已經有兄弟去巡城了，你就等改天吧！\n");

       //給任務，並且將該任務從列表中刪除
       obj->start_busy(1);
       PARTY_QUEST_D->delete_city_quest(PLACE,TYPE,bh);
       set("bunch_quest", bunch_quest, me);
       message_vision("$N對着$n點了點頭：那麼這個任務就交給你了，好好去干！\n",obj,me);
...
int do_giveup()
{
       ...
       if( !query("bunch_quest", me) )
               return notify_fail(obj->name() + "奇怪的看着你說：你現在並沒有任務在身啊，是不是閑的發慌了？\n");

       delete("bunch_quest", me);
       message_vision("$N對着$n一個勁的嘆氣，似乎不明白為什麼$n為何如此的不成材。\n",obj,me);
