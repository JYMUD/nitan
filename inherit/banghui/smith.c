...
void init()
{
	...
	add_action("do_epurate", "ti");	// 提煉、精鍊礦
...
int do_fix(string arg)
{
	...
	if(!arg || !ob = present(arg,me))
		return notify_fail(this_object()->name()+"看着你說道：“你要修理什麼？”\n");
	...
	if(ob->is_weapon())
	{
		...
		if(me->query_banghui_id() == b_id)
		{
			...
			write(sprintf("%s點點頭說道：%s修好了，拿着它為我們%s多出力。\n",
				name(), ob->name(), b_name));
			return 1;
		}
	...
	else
	{
		...
		if(me->query_banghui_id() == b_id)
		{
			...
			write(sprintf("%s點點頭說道：%s修好了，穿戴着它為我們%s多出力。\n",
				name(), ob->name(), b_name));
			return 1;
		}
...
int do_epurate(string arg)
{
	...
	if(do_nothing)
		return notify_fail(sprintf("%s說道：等會兒，正忙着呢。\n", name()));
	...
	if(ob->is_mine_stone())		// 提煉礦石
	{
		...
		if(me->query_banghui_id() != b_id)
		{
			...
			{
				destruct(tg);
				return notify_fail(sprintf("%s說道：提煉這塊礦石里的%s需要%s。\n",
					name(), cmcs, chinese_value(value)));
			}
		}

		do_nothing = 1;
		message_vision("$N朝着$n點點頭說道：等一會兒。\n", this_object(), me);

		call_out("epurate_it", 1+random(2), me, ob, tg);
		return 1;
	}

	else if(ob->is_iron_class_res())	// 精鍊
	{
		int upq;
		...
		|| !(upq = MINE_D->query_mine_class_up_quantity(mcs)) )
			return notify_fail(sprintf("%s搖搖頭說道：這東西沒法再精鍊了。\n", name()));

		if( (ew = ob->query_weight()/100) < upq )
			return notify_fail(sprintf("%s搖搖頭說道：對%s精鍊至少需要 %d 兩，這塊%s不夠。\n",
				cmcs, upq, cmcs));

		if(!objectp(tg = MINE_D->query_mine_class_up_ob(mcs)))
			return notify_fail(sprintf("%s搖搖頭說道：我這沒法對%s進行精鍊了。\n", name(), cmcs));
		...
		if(me->query_banghui_id() != b_id)
		{
			...
			{
				destruct(tg);
				return notify_fail(sprintf("%s說道：對這塊%s進行精鍊需要%s。\n",
					name(), cmcs, chinese_value(value)));
			}
		}
...
protected void epurate_it(object me, object ob, object tg)
{
	...
	rong_he(me, tg);
	do_nothing = 0;
	message_vision("$N朝着$n點點頭說道：提煉好了，拿去吧。\n", this_object(), me);
}
