  ...
  if (!me->query("family/family_name"))
   {
        ...
        if ( me->query("quest/book") && !me->query( "quest/book/finished" ) )
        {
                ...
                {
                        message_vision( CYN"$N對着$n說：你跑到哪裡閑逛去了？這次可要小心在意些！\n"CYN, master, me );
    ...
    dir[i]=="yanziwu" ||
                ...
                dir[i]=="npc")
        {
                message_vision("$N對着$n說：本門秘籍？不是好好的么？\n", master, me);
                return 1;
        }
        ...
        message_vision("$N對着$n說：本門秘籍流落江湖，聽說最近在" + to_chinese(dir[i]) + "一帶出現。你去把它取回來吧？\n", master, me);
...
int accept_quest_book(object me, object obj)
{
        ...
        if ( !me->query("quest/book") || me->query( "quest/book/finished" ) )
        {
                message_vision("$N對着$n說：我沒要這書，你去哪裡拿來的！\n", master, me);
                destruct(obj);
                return 1;
        }
        ...
        message_vision("$N對着$n說：本門秘籍？不錯，你為本門立了一大功！\n", master, me);
  ...
  if (!me->query("family/family_name"))
   {
                ...
                !me->query( "quest/betrayer/finished" ) )
        {
                ...
                {
                        message_vision( WHT"$N對着$n說：你跑到那裡閑逛去了？\n"NOR, master, me );
...
//先選地方 減少系統開支
  ...
  obj = create_betrayer(me);
        ...
        message_vision("$N對着$n說：本門歸山弟子回報，本門叛徒" + obj->query("name") + "在" + to_chinese(dir[i]) + "一帶出沒。你敢去把這叛徒的人頭給我取回來么？\n", master, me);
...
int accept_quest_betrayer(object me, object obj)
{
        ...
        {
                message_vision( HIR"$N對着$n說：我叫你殺叛徒，你卻濫殺無辜，蒙蔽師長！去死吧你！\n"NOR, master, me);
                destruct(obj);
...
//        p_bonus = (int)random(e_bonus/4)+35;
    ...
    me->add("family/gongxian",1);
        ...
        message_vision("$N對着$n說：叛徒的人頭？不錯，你為本門清理了門戶！\n", master, me);
  ...
  if (!me->query("family/family_name"))
   {
        ...
        if ( me->query("quest/thief") && !me->query( "quest/thief/finished" ) )
        {
                ...
                {
                        message_vision("$N對着$n說：怎麼樣，姦細"+ me->query("quest/thief/name")+"的人頭呢！？\n", master, me);
                        return 1;
                }
                else
                {
...
// 為了照顧南少林(shaolin是nanshaolin字符串的子集)，所以從後讀起
  ...
  !obj->move(room))
   {
           if (objectp(obj)) destruct(obj);
           message_vision("$N對着$n說：我這裡有點事要忙，等會兒我再看看吧。\n", master, me);
    return 1;
   }
        ...
        message_vision("$N對着$n說：本門弟子回報，我派周圍有陌生人出現。你去查看一下是不是別派的姦細？\n", master, me);
...
int accept_quest_thief(object me, object obj)
{
        ...
        if (obj->query("userp") == 1)
        {
                message_vision( HIR"$N對着$n說：我叫你殺姦細，你卻去殺玩家！去死吧你！\n"NOR, master, me);
                destruct(obj);
                quest_failed( me, "thief" );
                me->delete_temp("methiefnum");
                master->kill_ob(me);
        ...
        {
                message_vision( HIR"$N對着$n說：我叫你殺姦細，你卻濫殺無辜，蒙蔽師長！去死吧你！\n"NOR, master, me);
                destruct(obj);
                quest_failed( me, "thief" );
                me->delete_temp("methiefnum");
                master->kill_ob(me);
  ...
  e_bonus -= e_bonus/3;
        ...
        message_vision("$N對着$n說：姦細的人頭？不錯，你為本門立一大功！\n", master, me);
...
//                quest_failed( me, "thief" );
        ...
        {
                message_vision( HIR"$N對着$n說：我叫你殺人，你卻濫殺無辜，蒙蔽師長！去死吧你！\n"NOR, master, me);
                destruct(obj);
...
int do_qiecuo(string arg)
{
      ...
      //family_can_qiecuo作為師父獎勵和特殊perform切磋的區別
              if( !me->query_temp("family_can_qiecuo"))
              {
                ...
                if (fealty_b<learn_b)
                {
                        message_vision("$N對着$n說：你還是多為本門做點貢獻再來吧！\n",master,me);
                        return 1;
                }
                ...
                if( stringp(pfskill = me->query_skill_mapped(pfmartial)))
                {
                        ...
                        for ( j = 0; j < sizeof( perform_actions ); j++ )
                        {
                                ...
                                if ( perform_actions[j] == pfarg )
                                {
                                        ...
                                        {
                                                message_vision("$N對着$n說：你已經領會了這招，還要切磋么？\n", master, me);
                                                return 1;
                                        }
                                        else
                                        {
        ...
        else
        {
                      ...
                      if (!(special=me->query_skill_mapped(arg)))
                {
                  ...
                  if(arg=="force")
                {
                        message_vision("$N對着$n說：內功修為，不是靠切磋能提高的。\n", master, me);
                             return 1;
                }
