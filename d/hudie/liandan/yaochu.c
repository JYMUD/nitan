...
void create()
{
        set_name(HIC"葯鋤"NOR, ({ "yao chu"}));
        set_weight(100);
        if( clonep() )
                set_default_object(__FILE__);
        else {
                set("long", "一把葯鋤，用來在地里挖掘（wajue）根類藥材。\n");
                set("unit", "把"); 
                set("no_get",1);
                set("no_drop",1);             
                set("no_put",1);
...
int do_wajue()
{
  	...
  	object ob = this_object();
    	...
    	if( !objectp(weapon=query_temp("secondary_weapon", me) )
    	 || query("id", weapon) != "yaochu" )
        	return notify_fail("你手中得拿着工具才能挖葯。\n");

  	if( !query("outdoors", environment(me)) )
  	return notify_fail("你在房間里挖個什麼勁，要挖去外面挖！\n");
	...
	message_vision( HIB"$N拿起葯鋤，在地上一陣瘋狂亂刨！\n"NOR, this_player());
	return 1;
}
   ...
   set("wajue", time()+120, 		environment(me));
		...
		gen=new(__DIR__"/yao/genlei");
  		...
  		me->start_busy(2);
   		...
   		message_vision(HIR"$N拿着葯鋤刨地三尺，挖出了一根"+gen->name()+HIR"！\n"NOR, this_player());
	}
}