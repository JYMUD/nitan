...
int do_fangdan(string arg)
{
    ...
    if(!arg) return notify_fail("你要放什麼葯進去？\n");

    if( !query_temp("zhuyao", me) )
    return notify_fail("練丹要先放主葯,要不然練不出好丹!\n");
...
int do_zhuyao(string arg)
{
    ...
    if(!arg) return notify_fail("你要拿什麼東西當主葯？\n");
...
// By zjb@ty 煉丹的額外獎勵!
    ...
    set("zhuren", ""+query("id", me)+"", dan);
    dan->move(me);
    return notify_fail(HIG"你練出了療傷聖葯--祛腐生肌散!\n"NOR);
    }
    if (all==64 && zhuyao==22) {
    delete_temp("liandan", me);
    delete_temp("zhuyao", me);
    ...
    set("zhuren", ""+query("id", me)+"", dan);
    dan->move(me);
    return notify_fail(HIG"你練出了療傷聖葯--七厘丹!\n"NOR);
    }
    if (all==40 && zhuyao==23) {
    delete_temp("liandan", me);
    delete_temp("zhuyao", me);
    ...
    set("zhuren", ""+query("id", me)+"", dan);
    dan->move(me);
    return notify_fail(HIG"你練出了療傷聖葯--壯筋續骨丹!\n"NOR);
    }
    if (all==55 && zhuyao==24) {
    delete_temp("liandan", me);
    delete_temp("zhuyao", me);
    ...
    set("zhuren", ""+query("id", me)+"", dan);
    dan->move(me);
    return notify_fail(HIG"你練出了療傷聖葯--硃砂沒藥散!\n"NOR);
    }
...
// By zjb@ty 煉丹的額外獎勵!
   ...
   if ( zjb < 15 && zjb>10 && all2 < 150 && all2 > 40 
    ...
    set("zhuren", ""+query("id", me)+"", dan);
    dan->move(me);
    return notify_fail(HIY"你練出了療傷用的金創葯!\n"NOR);
    }
    ...
    set("zhuren", ""+query("id", me)+"", dan);
    dan->move(me);
    return notify_fail(HIW"你練出了少林聖葯菩提子!\n"NOR);
    }
    ...
    set("zhuren", ""+query("id", me)+"", dan);
    dan->move(me);
    return notify_fail(HIW"你練出了少林聖葯大還丹!\n"NOR);
    }
    ...
    set("zhuren", ""+query("id", me)+"", dan);
    dan->move(me);
    return notify_fail(HIW"你練出了補精聖葯,腎寶!!!\n"NOR);
    }
...
int do_yao(object me,object obj)
{
    ...
    if( query_temp("zhuyao", me) )
    return notify_fail("對不起你已經拿"+query_temp("zhuyaoname", me)+"做主葯了!\n");
