...
里黑洞洞的什麼也看不見，只有洞旁的一株奇異的小草，葉面閃爍着
點點的磷光，顯得鬼魅異常。
LONG );
...
int do_use(string arg)
{
        ...
        if ( arg == "fire" )
        {
                write( HIY "你點燃了火折，可是洞穴里實在太黑了，一支火折遠不"
                       "夠，看來要找支火把才行。\n\n" NOR);
                return 1;
        }
}
...
int do_climb(string arg)
{
        ...
        message_vision(HIC "$N" HIC "小心翼翼的攀着岩壁上突出的石筍爬了下去。\n\n" NOR,me);

        ob = load_object(__DIR__"wandu3");
        ob = find_object(__DIR__"wandu3");

        message("vision",HIC+query("name", me)+HIC"從岩壁上爬了下來。\n"NOR,ob);
...
力的痕迹，正中間有一個扁扁的小孔，好象可以插入(insert)什麼東
西試試。
LONG);
