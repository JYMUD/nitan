...
string long_desc()
{
        ...
        if( !query("exits/out") )
                ...
                {
                        case 1: desc += BLU"天空中彤雲密布，只有幾隻海鷗還在奮力展翅，海船左右搖晃不已，你感到有\n點立足不穩。\n"NOR; break;
        ...
        if( !(dir = (string)query_temp("navigate/dir")) ) {
                if( !random(100) ) {
                        ...
                        if( query_temp("navigate/wait") > 5 ) {
                                tell_room(this_object(), "船夫們把大家都扔進了海里。\n");
                                do_drop();
                                return 1;
                        }
...
void do_ready()
{
        ...
        delete_temp("trigger");
        delete("exits/out");
        message("vision", "船夫們大喝一聲“開船”，於是船便離了岸。\n", this_object());
}
