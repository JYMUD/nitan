...
int accept_object(object who, object ob)
{
        ...
        if( query("name", ob) != HIB"烏龜甲"NOR
                 && query("id", ob) != "fuling" )
                    return notify_fail("掌葯道長搖頭道：這不是配藥的材料。\n");

        if( query("name", ob) == HIB"烏龜甲"NOR )
        {
                if(query_temp("fuling")) {
                        write(MAG"掌葯道長大喜，龜甲和茯苓都有了，這副龜苓膏可煉成了。\n"NOR);
                        ...
                        if(query_temp("guijia")) {
                                write(YEL"掌葯道長搖了搖頭道：“龜甲我已經有了。”\n"NOR);
                                return 0;
                        }
                        else {
                                set_temp("guijia",1);
                                write(WHT"掌葯道長笑道：龜甲有了，就缺茯苓了。\n"NOR);
                                if(!(obj2=present("gui jia", this_player())))
                                destruct(obj2);
                                remove_call_out("destroying");
                                call_out("destroying", 1, me, ob);
        ...
        if( query("id", ob) == "fuling" )
        {
                if(query_temp("guijia")) {
                        ...
                        if(query_temp("fuling")) {
                                write(YEL"掌葯道長搖了搖頭道：“茯苓我已經有了。”\n"NOR);
                                return 0;
                        }
                        else {
                                set_temp("fuling",1);
                                write(WHT"掌葯道長笑道：茯苓有了，就缺龜甲了。\n"NOR);
