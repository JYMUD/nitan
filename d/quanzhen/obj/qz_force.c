...
int do_dazuo(string arg)
{
        ...
        if( query("sleep_room", where) )
                return notify_fail("不能在睡房裡修鍊內功，這會影響他人。\n");
...
int exercising(object me)
{
        ...
        {
                write("你默念着"+koujue[random(4)]+"，只覺得一股內息在體內迅速流動，全身都感到舒暢無比。\n");
        ...
        else
        {
                if( random(6)==3 )
                {
                        ...
                        {
                                write("你默念着"+koujue[random(4)]+"，只覺得一股內息在體內竄來竄去，讓你覺得不對勁，難道是走火入魔的前兆？\n");
                                cost = neili_gain + random(neili_gain*100);
                        }
                        else
                        {
                                write("你默念着"+koujue[random(4)]+"，只覺得一股內息在體內竄來竄去，讓你覺得不對勁。\n");
