...
void create()
{
        ...
        if (clonep())
                ...
                set("unit", "粒");
                set("long", "一粒淡褐色的草籽，種在田地里(zhong)會長成草藥。\n");
                set("value", 10);
        }
        setup();
}
...
int do_zhong(string arg)
{
        ...
        if( query("value", ob) != 10 )
                return notify_fail("已經種在地里了！\n");

        message_vision("$N輕輕將$n種在地里！\n", me, ob);
...
int grow_e(object ob, object me)
{
        ...
        if( MEMBER_D->is_valib_member(query("id", me)) )
        {
                ...
                {
                        message_vision(HIY "$N種植的草藥長成了，可以挖起來了(dig)。$N忽然發現上面好象懸浮着一道金光。\n"NOR, me);
                delete_temp("growing", me);
