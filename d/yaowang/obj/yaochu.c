...
void create()
{
        ...
        if (clonep())
                ...
                set("no_put",1);
                set("long", "這是一把很重的葯鋤，你可以用它來挖葯（dig yao）。\n");
                set("value", 5000);
        }
        setup();
}
...
{       object ob;
        ...
        if( query("qi", me)<40)return notify_fail("你太累了，怎麼都拿不動這把葯鋤！\n");
        if (present("zhengui yaocai", environment(this_object()))||present("putong yaocai",environment(this_object()))) return notify_fail("別不知足,你已經挖到葯了！\n");
        if(environment(this_object()) != me)  return notify_fail("你是在看別人挖吧？\n");
        if( query_temp("wayao1", environment(me)) )
        {
        me->start_busy(random(3)+1);
        message_vision("$N揮動葯鋤“呼哧、呼哧”地在地上挖了起來,似乎在找什麼東西。\n",me);
        addn("jing", -10, me);
        addn("qi", -20, me);
        if(random(7)<3)
        {
      ...
      return 1;
        ...
        message_vision(CYN"$N揮動葯鋤開始在地上挖了起來。\n"NOR,me);
        addn("kee", -20, me);
        addn("sen", -30, me);
             if(random(7)<4)
      {
