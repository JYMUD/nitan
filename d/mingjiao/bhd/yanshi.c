...
里是一個極大的島嶼。島嶼西部都是尖石嶙峋的山峰，奇開怪樣，莫
...
int do_push(string arg)
{
        ...
        if( arg=="chuan" || arg=="船" ) {
            ...
            me->start_busy(5);
            add("boat", -1);
            message_vision("$N使出吃奶的勁，將用樹榦製成的帆船推進了大海。\n\n", me);
...
int do_make(string arg)
{
        ...
        if( arg=="chuan" || arg=="船" ) {
            ...
            switch(i){
                case 0: if(!weapon || weapon->query("flag")!=4) return notify_fail("你需要一件鋒利的工具，才能剖開樹榦。\n");
                        msg = "$N將樹榦剖開，並排，再用藤條將它們牢牢地困緊。\n"; 
                        add("make_times", 1); break;
                case 1: msg = "$N將樹榦上的樹皮撥下來，打緊，再用騰條穿孔做成了帆。\n"; 
                        add("make_times", 1); break;
                case 2: msg = "$N將樹榦掏空，打磨了一翻，然後用掏出的木頭做成了漿，舵等用具。\n"; 
                        add("make_times", 1); break;
                case 3: msg = "$N將一根樹榦削圓削細，仔細量了尺寸，做成了主桅杆。\n"; 
                        add("make_times", 1); break;
...
{   
       ...
       if(boat)
          msg += "岸邊停放着一隻簡陋的帆船，正隨波左右搖動。從這裡就可以上去(enter)。\n";        
          msg += "這裡寬闊平坦，看來可以是個很好的工場。\n";
       if(query("make_times") >= 1){ 
            if(query("make_times") >= 4) msg += "有一隻用巨樹樹榦製成的簡陋帆船的船體，看來是快要完成了。\n";
            else msg += "一旁放有幾根巨樹的樹榦，已經被剖開成了兩半。\n";  
       }
       if(query("boat")) msg += "岸邊放有用巨樹樹榦製成的簡陋帆船，如果使勁推，可以把它推(tui)下水去。\n";
       return msg;       
}
