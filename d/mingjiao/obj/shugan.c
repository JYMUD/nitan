...
void create()
{
        set_name("樹榦", ({ "shu gan","trunk"}) );
        set_weight(20000);
        if( clonep() )
                set_default_object(__FILE__);
        else {
                set("long", "這是一根剛剛砍下來的樹榦。\n");
                set("unit", "根");
                set("no_get", "這麼大的樹榦，你能拿起來嗎？");
                set("value",1);
        }
}
...
int do_carry(string arg)
{
        ...
        if( query("owner") != query("id", me) )
                if (present(query("owner"),environment(me)) )
                {
                        ...
                        message_vision("$N偷偷地想把樹榦抬起來，突然$n瞪了$N一眼，嚇地$N趕緊縮手。\n",me,owner);
                        return 1;
                }
                else
                {
                        message_vision("$N瞧了瞧旁邊沒人，偷偷摸摸地把樹榦扛到肩上。\n",me);
                        this_object()->move(me);
                        set_temp("apply/short", ({(query("title", me)+""+query("name", me)+"扛着一根樹榦")}), me);

                        return 1;
                }
        
        message_vision("$N使勁一抬，將剛砍下來的樹榦扛到肩上\n",me);
        this_object()->move(me);
        set_temp("apply/short", ({(query("title", me)+""+query("name", me)+"扛着一根樹榦")}), me);
