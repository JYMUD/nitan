...
int ask_snake()
{
        ...
        if( query("family/family_name", me) != "歐陽世家"){
                command("say "+RANK_D->query_rude(me)+"想打聽咱們的蛇兒，莫非動着什麽歪腦筋？");
                return 1;
        }

        if( query("family/master_name", me) != "歐陽鋒"){
                command("say 場子里的蛇都是莊主和少主所飼養的。");
                return 1;
        }

        if( !query("baituo/new_breed_time", me) && !query("baituo/checked_snake", me)){
                command("say 場子里沒有少主養的蛇，您是也想養些麽？");
                return 1;
        }
...
int ask_guaishe()
{
        ...
        if( query("family/family_name", me) != "歐陽世家"){
                command("say "+RANK_D->query_rude(me)+"想打聽咱們歐陽世家的怪蛇，莫非動着什麽歪腦筋？");
                return 1;
        }
        ...
        if ( num < 1)
                command("say 現下場子里沒有少主養的怪蛇。");
        else command("say 少主養在場子里的怪蛇，一共有"+chinese_number(num)+"條。");
...
int do_order(string arg)
{
        ...
        else return notify_fail("你要對蛇奴下什麽命令？ (defense or remove) \n");
