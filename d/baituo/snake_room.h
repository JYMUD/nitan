...
int do_search(string arg)
{
        ...
        if(arg == "grass" && query("grass") ) {
                ...
                place = "洞";
                message_vision("$N用"+weapon->name()+"在洞口地面托、托地輕敲着。\n", me);
                addn("jingli", -10, me);
        }
        ...
        else return notify_fail("找了這麽久，你覺得"+place+"中應該是不會藏着蛇了。\n");
...
void check_snake(object me, string place)
{
        ...
        else message_vision(place+"中傳出一陣細微的聲響，似乎有什麽東西，但是沒有出來。\n"NOR, me);
...
int do_burn(string arg)
{
        ...
        else return notify_fail("在這裡燒樹枝也沒什麽用。\n");

        message_vision(HIR"$N將"+branch->name()+HIR"堆在"+place+"前，用火折點燃。\n"NOR, me);
        message_vision(WHT"一股濃煙慢慢湧入"+place+"里........\n\n"NOR, me);
...
void smoke_snake(object me, object branch, string place)
{
        ...
        if(query("hide_snake") < 1 ) {
                message_vision("煙熏了老半天，樹枝都燒成灰了，"+place+"里卻一點動靜也沒有。\n", me);
                destruct(branch);
                return;
        }
