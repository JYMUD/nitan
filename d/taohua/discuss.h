        ...
        if( sscanf(arg, "%d", ans)==1 ) {
                if( ans==answer ) {
                        ...
                        if( query_temp("wrong/"+query("id", this_player()))>3){
                                ...
                                message_vision(HIC"黃藥師袖袍一揮，把$N凌空捲起，走到屋外，對啞仆喊道：“你們把這不肖之徒扔到外島去，越遠越好！”\n\n黃藥師氣哼哼的走了。\n\n"NOR,ob);
...
*/
        ...
        if( query("th_discuss", me)>2000 )
                return notify_fail("你討論了這麽多，應該實踐一下本門武功的技巧了。\n");
...
*/
        ...
        if( query("jing", me)<10+10*times || query("jingli", me)<10+10*times )
                return notify_fail("以你目前的精神狀況，怕一次討論不了這麽多東西。\n");
                  if( query("jing", partner)<10+10*times || query("jingli", partner)<10+10*times )
                                         return notify_fail("以對方目前的精神狀況，怕一次討論不了這麽多東西。\n");
        ...
        if( exp1 < 1 ) {
                ...
                tell_object(partner,CYN""+name1+"似乎想和你討論點什麽的樣子...\n"NOR);
                ...
                return 1;
                  ...
                  if( exp2 < 1 ) {
                                         ...
                                         tell_object(partner,CYN""+name1+"似乎想和你討論點什麽的樣子，但你卻覺得沒啥好說的...\n"NOR);
                                         message_vision(CYN"$n對$N一副欲言又止的尷尬模樣。\n"NOR,me,partner);
                                         delete_temp("exp_gain", me);
                                         return 1;
                  }
...
/*
  if( random( 150 ) == 1 && bCheckingRobot == 0)
  {
        ...
        if( query("th_discuss", me)>2000){
        message_vision("$N正欲討論武學精要，一個啞仆走了過來，對$N咿咿呀呀地做了個手勢，是黃藥師有事要找$N，不知有何吩咐。\n",me,partner);
        message_vision("$N和$n對望一眼，停了下來。$N向啞仆揮揮手，示意知道，啞仆便即離去。\n",me,partner);
