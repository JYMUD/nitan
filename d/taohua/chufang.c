...
void create()
{
        set("short", "廚房");
        set("long", @LONG
這裡常年有幾個啞仆在烹調食物，等候傳喚。對待這些人不用太客氣，
命令(order)他們就可以了。
LONG );
        set("exits", ([
...
int do_order()
{
        ...
        message_vision("$N打手勢命令一個啞仆拿些食物吃喝。\n", me);
        ...
        && !present( "fish", environment(me) ) ) {
           ...
           case 2:
                ...
                ob1->move(me);
                message_vision("啞仆面無表情地拿出"+ob1->name()+"給$N。\n", me);
        }
        else if ( (int)me->max_food_capacity()-food_ind >= 40 )
                message_vision("啞仆指了指$N拿着的食物，搖了搖頭。\n", me);
        else message_vision("啞仆指了指$N的肚子，搖了搖手。\n", me);

        if ( (int)me->max_water_capacity()-water_ind >= 40 && !w
        && !present("tea", environment(me)) ) {
                ob2 = new(__DIR__"obj/xiangcha");
                ob2->move(me);
                message_vision("啞仆面無表情地拿出"+ob2->name()+"給$N。\n", me);
        }
...
int valid_leave(object me, string dir)
{
        ...
        if ( (f>0 || w>0) && present("ya pu", environment(me)) ) 
                return notify_fail("啞仆攔着你，指了指你拿着的食物，搖了搖頭。\n");
