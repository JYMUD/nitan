...
void create()
{
        ...
        set("long", "一個二十齣頭的小夥子，身板結實，雙目有神，似乎練過幾年功夫。\n");
        set("gender", "男性");
        set("age", 25);
...
void leave(object me)
{
        ...
        message_vision(CYN"$N說道：這麽久還捉不到那"+query("find_yapu_real")+"裝成的"+query("find_yapu")+"，我看今天是逮不到他了，不如先回歸雲庄吧。\n"NOR,jiading);
        command("sigh");
...
int do_recognize(string arg)
{
        ...
        set("real_message", HIR"小販臉色一變，喝道："+RANK_D->query_rude(me)+"原來是黃老邪門下？想捉"+RANK_D->query_self_rude(ob)+"，可沒那麽容易！\n"NOR, ob);
        ...
        message_vision(HIR"隨着哨子聲，玉笛里突然飛出藍印印的一點火星，火星陡地熄滅，隨即大亮，蓬的一聲響，騰向半空，升起有丈許，這才緩緩降落。\n"NOR, me);
        ...
        {
        message_vision("家丁獃獃地看著$n……\n",me,ob);
        command("shake");
        command("say 這位"+ RANK_D->query_respect(me) +"神仙一般的人物，怎麽會是那個"+query("find_yapu_real")+"？");
        }
        else if( query("gender", ob) == "男性" && query("per", ob) >= 30 )
        {
        command("shake");
        command("say 這位"+ RANK_D->query_respect(me) +"儀錶堂堂，豈是那"+query("find_yapu_real")+"可以比擬？");
        }
        else
        {
        command("shake");
...
int do_bring(string arg)
{
        ...
        if(!objectp(ob = present(arg, environment(this_player()))))
                return notify_fail("這裡沒有這個人。\n");
        if(ob == me)
                return notify_fail("帶走自己？你到底在想什麽？\n");
        if(ob == jiading)
        ...
        {
        command("shrug");
        command("say 怎麽把人打死了？……算了，還是先把它帶回去交差吧。");
        ...
        destruct(jiading);
                ...
                message_vision(CYN"$N說道：你先別動手，直接把$p帶回歸雲庄，交給"+lu+"，由他處理便是。\n"NOR,me,ob);
                command("ok");
                ...
                message_vision(HIR"家丁掏出一把細長的鐵柄小刀，扳開$n的嘴巴，飛快地斬去了$n的舌頭，跟着轉過刀鋒，在$p雙耳各刺一刀，立時鮮血泉涌！\n"NOR,me,ob);
                message_vision(HIR"家丁收起小刀，取出一個瓷瓶，挑出傷葯彈在$p斷舌和雙耳傷處，流血立緩，可是從此也已成聾啞之人。\n"NOR,me,ob);
                addn("th_exp", yapu_exp, me);
                addn("th_pot", yapu_pot, me);
                }
                else {
