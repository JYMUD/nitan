...
void create()
{
        set("short", "中央廣場");
        set("long", @LONG
這裡是城市的正中心，一個很寬闊的廣場，鋪着青石地面。游手
好閑的人常在這裡溜溜達達，經常有藝人在這裡表演。中央一棵大榕
樹(tree)盤根錯節，據傳已有千年的樹齡，是這座城市的歷史見證。
樹榦底部有一個很大的樹洞(dong)。你可以看到北邊有來自各地的行
...
int do_enter(string arg)
{
        ...
        if (arg == "dong")
        {
                ...
                message("vision", HIC + me->name() + HIC "一彎腰往洞里走"
                                  "了進去。\n" NOR, environment(me), ({me}));

                me->move("/d/gaibang/inhole");
                message("vision", HIC + me->name() + HIC "從洞里走了進來"
                                  "。\n" NOR, environment(me), ({me}));
                return 1;
        }
}
