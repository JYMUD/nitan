   ...
   else
   {
                        ...
                        switch(random(4))
                        {
                                case 0:
                                        ...
                                        break;
                                case 1:
                                        message_vision( HIR"$n陰笑一聲說道，這麽多不怕死的？再來十個大爺我也不怕！\n隨即眼睛一瞪，向著$N就沖了過去！\n"NOR,ob,me);
                                        me->set_leader(ob);
                                        remove_call_out("kill_ob");
                                        call_out("kill_ob", 0, ob);
                                        delete_temp("mark/gkill1", ob);
...
// end///  第二次追殺
                if( query_temp("mark/gkill2", ob) )
                {
                        ...
                        {
                                case 0:
                                        message_vision( HIR"$n額頭青筋跳動，冷哼了一聲，瞪着通紅的眼睛向著$N就沖了過去。\n"NOR, ob,me);
                                        me->set_leader(ob);
                                        if ((int)exp<3000000)
                                           set("combat_exp", exp*3/2, me);
                                        else
...
int do_yes()
{
        ...
        {
                command( "say 您老人家功夫如此出眾，小的實在想不出拿什麽孝敬您老，回頭我想起在說吧。\n");
                message_vision("$N嘿嘿一笑，轉身離開，一眨眼就消失不見了。\n",me);
                destruct(me);
                return 1;
        }
...
int do_no()
{
        ...
        command("say 這位"+RANK_D->query_respect(ob)+"一身正氣，不愧名門正派弟子，不過不知道功夫怎麽樣？！\n");
