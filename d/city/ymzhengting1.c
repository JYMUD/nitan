...
int do_work (string arg)
{
        ...
        message_vision("程葯發一拍驚堂木怒道：據查今有"+kf_name+"在"+kf_where+kf_name1+"，"+query("name", me)+"速去破案，不得有誤。\n",me);
        set("kf_working", 1, me);
        return 1;
}
int do_finish (string arg)
{
        ...
        if( !query("kf_working", me) )
                return notify_fail("程葯發臉色一沉道：本官什麼時候派過你差事啊？\n");
        if( !query_temp("kf_finish", me) )
                return notify_fail("程葯發臉色一沉道：本府交你的差使至今還未辦妥，你還有面目來見我？\n");
        if (present("ling pai",me)){
                tell_object(me, "你恭恭敬敬的繳上令牌。\n" NOR );
        destruct(present("ling pai",me));
        }        
        message_vision(WHT"程葯發微微一笑道："+query("name", me)+WHT"在"+kf_where+WHT"破獲"+kf_name+WHT"一案，勞苦功高，趕快下去休息吧！\n"NOR,me);
        ...
        if( query("kf_worked", me) == 4800){
                ...
                message_vision(WHT"程葯發微笑着說道："+query("name", me)+"連破大案，天下聞名，本府已經奏明皇上。\n昨日朝旨已下，獎你黃金五千兩。你可以到錢莊領取。"NOR,me);
        ...
        if( query("kf_worked", me) == 3200){
                ...
                message_vision(HIC "$N的正氣提高了！\n" NOR,me);
                tell_object(me,"你現在的正氣是"+chinese_number((query("shen", me)))+"。\n"NOR);
                message_vision(WHT"程葯發微笑着說道："+query("name", me)+"連破大案，天下聞名,繼續加油！！"NOR,me);
      }
