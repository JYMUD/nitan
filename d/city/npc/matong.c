...
string *MjC = ({"",
        ...
        "1 春","2 夏","3 秋","4 冬","1 梅","2 蘭","3 竹","4 菊","暗杠 "
});
int K;
string Tc = "";
//資料
...
int do_gang(string str,object me)
{
        ...
        if (i==1)
        {
                ...
                if (!i) return notify_fail("您只有一張不能杠。\n");
                if (NO_GANG) return notify_fail("這樣可杠不了。\n");
                message_vision("\n$N說道："+HIB+"木工"+NOR+"～～～\n\n",me);
                who["Mj"]=do_delete_Mj(who["Mj"],str,1);
        ...
        if (i < 3)
                return notify_fail("你手中要有三張"+do_check_num(str,0)+"才能杠。\n");
        if (!Touched)
        {
                if (str!=LastDump) return notify_fail("這樣可杠不了。\n");
        }
        else if (i < 4)
                        return notify_fail("你手中要有四張"+do_check_num(str,0)+"才能暗杠。\n");
        for (i=0;i<Play;i++)
        {
...
int do_peng(string str,object me)
{
        ...
        i=do_check_Mj(who["Mj"],str);
        if (i==0)
                return notify_fail("你沒"+do_check_num(str,0)+"這張牌要怎麽碰。\n");
        if (i < 2)
...
int do_setmj(string str)
{
        ...
        if (!str)
        {
                ...
                if (who["AutoGang"]!="") numstr+="遇到"+HIY+show_mj(who["AutoGang"],2)+NOR+"會自動"+HIY+"[杠牌]"+NOR+"。\n";
        ...
        if (str=="ting")
        {
        ...
        else if (str=="gang")
                {
                        ...
                        if (check < 3)
                                return notify_fail("你手中要有三張"+do_check_num(numstr,0)+"才能設定杠此牌。\n");
...
int do_look(string arg)
{
        ...
        if (arg=="mj")
        {
                ...
                if (who["AutoGang"]!="")
                        Mj+="遇到"+show_mj(who["AutoGang"],2)+"會自動杠牌、";
                if (who["AutoPeng"]!="")
