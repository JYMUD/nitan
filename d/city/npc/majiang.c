...
int do_pause( object me );        //加入停下來讓玩家考慮是否碰牌，杠牌等。
...
"Pause": "", "H2Ps": "", "H2Es1": "", "H2Es2": "", "H1s": ""                //為了判斷碰和吃及杠而加
]);
mapping P2Data = ([
...
//        tell_object(me,"換誰打牌:"+NowPlayerId[NowPlayer]+"\n");
        ...
        if (Goned==5){
           message_vision("$N說道：此局因"+HIR+"五杠合牌"+NOR+"請再重新開始 start 。\n",this_object());
           Playing=0;
           Count++;
           if (Play==4){
              for (i=0;i<Play;i++){
...
int do_gon(string str,object me)
{
        ...
        if (i==1){
           ...
           if (!i) return notify_fail("你只有一張不能杠  。\n");
           if (NO_GON) return notify_fail("這樣無法杠牌。\n");
              message_vision("\n$N說道："+HIB+"木工"+NOR+"～～～\n\n",me);
              who["Mj"]=do_delete_Mj(who["Mj"],str,1);
              destop+=str;                //杠完這牌當然要到桌面上來咯
        ...
        if (i < 3) return notify_fail("你手中要有三張"+do_check_num(str,0)+"才能杠。\n");
        if (!Touched){
           if (str!=LastDump) return notify_fail("這樣無法杠。\n");
        }
        else if (i < 4) return notify_fail("你手中要有四張"+do_check_num(str,0)+"才能暗杠。\n");
        for (i=0;i<Play;i++) {
...
int do_pon(string str,object me)
{
        ...
        if (who["Auto"]=="auto") return 1;
        i=do_check_Mj(who["Mj"],str);
        if (i==0) return notify_fail("你沒"+do_check_num(str,0)+"這張牌要怎麽碰。\n");
...
int do_setmj(string str)
{
        ...
        if (!str){
          ...
          if (who["AutoGon"]!="") numstr+="遇到"+HIY+show_mj(who["AutoGon"],2)+NOR+"會自動"+HIY+"[杠牌]"+NOR+"。\n";
        ...
        if (str=="ting"){
           ...
           if (numstr=="none"){
              who["AutoGon"]="";
              return notify_fail("你取消自動杠牌設定。\n");
           }
           check=do_check_Mj(who["Mj"],numstr);
           if (check==0) return notify_fail("你沒"+do_check_num(numstr,0)+"這張牌。\n");
           if (check < 3) return notify_fail("你手中要有三張"+do_check_num(numstr,0)+"才能設定杠此牌。\n");
           check=do_check_Mj(who["AutoGon"],numstr);
           if (check){
              who["AutoGon"]=do_delete_Mj(who["AutoGon"],numstr,1);
              return notify_fail("你"+HIR+"刪掉了"+NOR+"遇到"+do_check_num(numstr,0)+"會自動杠牌的設定。\n");
           }
           who["AutoGon"]+=numstr;
           Auto_Gon=1;
           write("你設定遇到"+do_check_num(numstr,0)+"時會自動杠牌。\n");
           return 1;
        }
        else if (str=="peng"){
           if (numstr=="none"){
        ...
        for(i=0; i<3; i++) {
                ...
                if( do_check_win(who["Mj"]) ) {                        
                        if( !userp(ob) ){
                                ...
                                tell_object(ob, "是否胡或碰或杠"+ do_check_num(LastDump, 0)+"？？（如果是，請輸入：hu/peng/gang）\n如果否，請輸入continue\n");
                ...
                if( !userp(ob) ) {
                        ...
                        if( do_check_Mj(next_data(query("id", me),i,"Mj"),LastDump)>2){
                                ...
                                PAUSE = 2;        //把時間弄停，防止玩家對戰時搶摸……
                                tell_object(ob, "是否碰或杠"+ do_check_num(LastDump, 0)+"？？（如果是，請輸入：peng/gang）\n如果否，請輸入continue\n");
                                who["Pause"] = "yes";                                
                                return 1;
                        }
