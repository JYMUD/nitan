...
// 重量決定食量
        ...
        message_vision("$N看了一眼"HIY"黃金"NOR"成色，伸手一捋，把它捋到抽屜里。然後從身後籠子里取出一隻"+title[num]+NOR"放在地上。\n", ob);
...
int do_lingqu()
{
        ...
        for (i=0; i<sizeof(obj); i++)
        {
                        ...
                        query("curiousness", obj[i]) )
                {
                        if(environment(obj[i]) == me )
                                return notify_fail(ob->name()+ "說道：你不是正騎在你的寵物身上么？\n");
                        else
                                if(environment(obj[i]) != environment(me) )
                                        ...
                                        return notify_fail(ob->name()+ "說道：你的寵物不是就在你旁邊么？\n");
                        return 1;
                }
        }
        pet = new("/clone/misc/pet");
        ...
        pet->move(environment(me));
        message_vision("$N從身後籠子里取出一隻"+pet->name()+"放在地上。\n", ob);
        me->start_busy(1);
        return 1;
}
