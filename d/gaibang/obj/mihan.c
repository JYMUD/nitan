...
int response(object me, object who, object obj)
{
        ...
        if( query("target_id", obj) != query("id", who )
         || query("target_name", obj) != who->name()){
                ...
                if( query("shen", who)<0){
                        ...
                        message_vision("$N將密函還給$n，鄭重的說道：唉！你怎麽如此大意！要讓惡徒將消息走露那可怎麽辦？\n", who, me);
                ...
                message_vision("$N抬起頭來，疑惑地說道：這信不應由你送來，你是怎麽得來的？\n", who);
                ...
                if( query("shen", who)<0){
                        ...
                        if( query("shen", me)>0 && random(query("kar", me))>10){
                                ...
                                message_vision("$N拍着$n肩膀高聲說道：幹得好！這密函我留下了！你的功勞我記着便是。\n", who, me);
                        }
        ...
        if( query("shen", who)<0){
                ...
                if( random(query("kar", me))>10){
                        ...
                        message_vision("$N奸笑道：我正愁不知要送甚麽給"+boss+"以表達我的心意，你自個兒送上門來，可怨不得我！\n", who);
...
//                        me->start_call_out( (: call_other, __FILE__, "check_fight", me, obj :), 1);
                        ...
                        message_vision("$N沉聲對$n說道：快滾！甚麽狗屁國家民族大義？這世上只有金銀權位才是好東西！\n", who, me);
                ...
                switch(random(3)) {
                case 0:
                        message_vision("$N說道：請告知魯長老說他交待的大事，我屆時一定會儘力，以盡俠義道的本分！\n", who);
                        break;
                case 1:
