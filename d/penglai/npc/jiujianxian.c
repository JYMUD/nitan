...
string ask_jiu()
{
       ...
       */
        ...
        message_sort(HIR "\n$N" HIR "從腰間取出一個葫蘆，將各種材料放進葫蘆中，然後閉上眼睛，口中"
                     "默念着什麼，似乎正在施展法術 …… 頃刻，一股酒香從葫蘆中飄出。看來這青梅酒已成。\n\n" NOR, this_object(), me);

        command("say 好了好了，很久沒有聞到這青梅酒的香味了。");
        
        if (! me->query("penglai/qingmeijiu_quest/ok"))
        {
                command("say 本劍仙忘記告訴你了，凡是第一次找我釀製青梅酒後，這酒都由本劍仙代為品嘗了 ……");
                message_vision(HIC "說罷，$N" HIC "脖子一仰，將葫蘆里的青梅酒喝了個精光。\n" NOR, this_object());
                command("haha");
                command("say 好酒，好酒啊。");
