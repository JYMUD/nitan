...
//      if(query("bee_out")) return notify_fail("你的玉蜂群忙着呢！\n");
        ...
        message_vision(HIG"$N輕輕地打開玉蜂箱的門，玉蜂陸陸續續地飛出來，采蜜去了。\n"NOR,me);
...
int auto_check(object me,object ob)
{
        ...
        if( query("mi", ob)>25 && random(2) == 0 || 
            query("mi", ob) >= 40 )
        {
                message_vision(HIW"玉蜂團團打轉，看樣子蜂箱的蜜已經采滿了,你應該回去了。\n"NOR,me);
                return 1;
        ...
        else if (random(15) == 0)
        {
                if (random(3) == 0)
                {
                        message_vision("$N看見一些玉蜂在一個地方奇怪地舞着，也許有什麼藥材可以采(cai yao)吧。\n",me);
        ...
        {
                message_vision(HIY"$N嘴裡不斷發出嗡嗡聲，指導玉蜂更有效率的采蜜。\n"NOR, me);
    ...
    if (!objectp(who = present(target, environment(me))) || !living(who))
        ...
        if( query("owner", obj) == me )
        {
                if(query("escaped")) {
                        ...
                        message_vision(CYN"$N紅着臉對孫婆婆說道，這次工作沒幹好。\n"NOR, me, who);
