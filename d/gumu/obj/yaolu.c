...
#define _DES "一隻古墓派特製葯爐，青玉石製成，樸實無華。\n爐頂有個石蓋(lip)，蓋中偏左有一孔，爐下有炭盆，盆內的木炭(coal)看來是新加的。\n"
#define _FIRE HIR"\n葯爐下正燃着火。\n"NOR
#define _OPEN "葯爐蓋子已經揭開。\n"

void create()
{
        set_name(CYN"青石葯爐"NOR,( {"yao lu","yaolu","lu"} ));
        set_weight(10000);
        if (clonep() )
...
int do_open(string arg)
{
        ...
        if (query("is_burning")){
                message_vision(RED"只聽「啊」的一聲慘叫，原來$N被葯爐燙了手。\n"NOR,me);
                addn("qi",-150+random(50), me);
                set("long",_DES+_FIRE);
                return 1;
        }
        
        message_vision("$N將葯爐蓋子揭開。\n", me);
        set("long",_DES+_OPEN);
        set("open", 1);
...
int do_addyao(string arg)
{
        ...
        if (item == "all" ) return notify_fail("這小小葯爐怎麼裝得下這麼多東西？\n");
                
        if (!objectp(obj = present(item, me))) return notify_fail("東西呢？\n");
                                                                
        if( !query("yaocai", obj))return notify_fail("葯爐里只能放用以煉製的藥材。\n");
        ...
        message_vision("$N把一"+query("unit", obj)+query("name", obj)+"放進葯爐。\n",me);
        destruct(obj);
        addn("count",-1);
...
int do_pour(string arg)
{
        ...
        if ( lvl < 61
                || ( lvl < 91 && arg == "玉蜂王漿" )
                || ( lvl < 121 && arg == "古墓玉漿" ))
                return notify_fail("你的內功修為不夠，這種葯熬不出，還是別浪費了.\n");
        ...
        if ( arg == "玉蜂毒" ){
                ...
                addn("cure_d",0);
                addn("cure_n",-80);
                message_vision("$N從"+query("name", box)+"中抓了幾隻玉蜂，滴了幾滴玉蜂毒在葯爐里。\n",me);
                return 1;
        }
        ...
        message_vision("$N從"+query("name", ping)+"中倒了一些"+arg+"在葯爐里。\n",me);
        return 1;
}
...
int do_burn(string arg)
{
        ...
        if (!(query("ready"))) return notify_fail("葯爐里什麼都沒有。\n");
...
void burning(object me)
{
        if (!query("mi")){
                set("long","一隻小巧玲瓏的煉藥爐。\n");
                message_vision(HIR"突然間葯爐內火光大盛，黑煙冒出，$N急忙將火頭撲滅。\n"NOR,me);
...
int do_aoyao()
{
        ...
        message_vision(HIC"$N一手靠近爐火一抽一放，借內力操控火勢；一手握住葯杵，從蓋上孔中伸入\n不停攪拌。\n"NOR,me);
        message_vision(HIR"\n$N不停的重複運動，一會兒已是滿身大汗。\n"NOR,me);
...
int do_quyao(string arg)
{
        ...
        if (!query("drug") || query("drug") == "none")
                ...
                destruct(zhen1);
                zhen2->move(me);
                message_vision("$N取出金針蘸了蘸葯爐中熬好的玉蜂毒，放到火上淬火……\n", me);
                message_vision("只聽得……，一縷青煙裊裊升起，$N詭異的笑容映着爐火忽明忽暗。\n\n", me);
        }
                ...
                message_vision("$N將小心地把葯爐里的"+query("drug_name")+"斟入青瓷瓶。\n\n", me);
        }
        else{
                ob = new(FILE_DIR+query("drug"));
                message_vision("$N從葯爐中取出一"+query("unit", ob)+query("name", ob)+"\n\n",me);
                ob->move(me);
        }
