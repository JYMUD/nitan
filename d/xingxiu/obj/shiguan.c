...
void over_liandu(object ob)
{
        ...
        if(objectp(corp = present("corpse", ob)))
        {
                if( query_temp("liandued", ob) )
                        message_vision("\n$N"RED"里腐爛的屍體猶如一隻泄了氣的皮球一樣，慢慢開始萎縮，最後只剩下一小片血跡在棺底！\n"NOR, ob);
                else message_vision("\n$N"RED"里腐爛的屍體“膨”的一下爆裂開來，濺得到處都是血跡！\n"NOR, ob);
                destruct(corp);
...
上角，以屈下頰至；又從耳後入耳中，出走耳前，交頰。三路合於一處！\n"NOR);
        ...
        case 51:
                ...
                break;
        case 61:
                write(BLU"入任脈，起於中極之下，以上毛際，循腹里，上關元，至咽喉，上頤循面，入目絡舌！\n"NOR);                
                break;
        case 71:
                write(BLU"入陽脈者，起於跟中，循外踝，上行入風池！\n"NOR);                
                break;
        case 81:
                write(BLU"入陰脈，起於跟中，循內踝，上行至咽喉，交貫沖脈！\n"NOR);                
                break;
        case 91:
        ...
        case 121:
                ...
                break;
        case 141:
                write(BLU"入沖脈者，起於氣沖，並足陽明之經，夾臍上行，至胸中而散也！\n"NOR);                
                break;
        case 161:
        ...
        if( query("neili", me)<0 || query("jingli", me)<0 )
        {
                ...
                tell_room(environment(me), GRN"突然間，你看見"+me->name()+"全身發顫，一股青氣竟然從石棺里自下而上直衝到"+me->name()+"的前胸！\n"NOR,({ me }));
                set("neili", 0, me);
