...
int do_vip(string arg)
{
        ...
        if (sscanf(arg,"%s -c",arg) == 1)
        {
                if (member_array(arg,vip) == -1)
                        return notify_fail(ob->name(1) + "賠着笑臉說：" + arg + "本來就不是您的貴賓啊！\n");
                vip -= ({arg});
                set("vip", vip, ob);
                ob->save();
        ...
        if( sizeof(vip) >= query("max_vip", ob) )
                return notify_fail(ob->name(1) + "苦着臉說：您定的貴賓太多了，老奴我實在是記不住啊！\n");

        if( arg == query("id", me) )
                return notify_fail(ob->name(1) + "一個哆嗦，戰戰兢兢的說：您是主人啊，老奴不敢當您是客人！\n");

        if (member_array(arg,vip) != -1)
                return notify_fail(ob->name(1) + "賠着笑臉說：" + arg + "本來就已經是您的貴賓啦！\n");
