...
//        set("apprentice",1);
        ...
        set("inquiry", ([
                "name": "在下陸冠英，是這裡的少莊主。",
                "rumors": "聽說蒙古兵又要南侵了，唉！",
                "here": "這裡是歸雲庄，你隨便轉轉吧，累了請到客房休息。",
                "東邪": "那是師祖呀，，蒙他老人家恩准，我才能跟父親學藝。",
                "黃藥師": "那是師祖呀，，蒙他老人家恩准，我才能跟父親學藝。",
                "桃花島": "聽說是師祖住的地方，可惜沒有去看過。",
                "陸乘風": "是我的父親，找他老人家有什麼事嗎？",
                "陸冠英": "不敢，就是在下。",
                "歸雲庄": "這裡是花了我父親無數心血才建起來的，與別的莊院相比如何？",
                "太湖": "就在庄前不遠，是一個煙波浩淼的大湖。",
                "葯" : "爹爹讓我保管靈藥，桃花島弟子可以到我這裡領取。",
//                "三才聚精丹": (: ask_yao1 :),
                "六壬集氣丸": (: ask_yao2 :),
...
string ask_yao1()
{
        ...
        if (ob->query_condition("jingli_drug") > 0 )
                return RANK_D->query_respect(ob) + "你不是剛吃過葯嗎，怎麼又來要了？靈藥多吃有害無宜，過段時間再來吧。";
        ...
        if (query("yao") < 1) return "對不起，葯已經發完了，新的還未煉出。";
        ...
        return "好吧，此葯練之不易，對練武之人大有好處，你要小心收藏好了。";
}
...
string ask_yao2()
{
        ...
        if (query("yao") < 1) return "對不起，葯已經發完了，新的還未煉出。";
         ...
         if( query("max_neili", ob) >= (ob->query_skill("force")*8+query("combat_exp", ob)/1000) )
                     return RANK_D->query_respect(ob) + "此葯已經難以在幫助你提高內力修為了！";
