...
void attempt_apprentice(object ob)
{
        if (ob->query_dex() < 20)
        {
           ...
           command("say 這位"+RANK_D->query_respect(ob)+"可能是白來歸雲庄一趟了。");
           return;
        }
        if( query("shen", ob)<0 )
        {
...
string ask_me()
{
        ...
        if( query("family/family_name", me) != "桃花島" )
           return "歸雲庄就是在下居處，如果"+RANK_D->query_respect(me)+"有意(decide)，我可修書一封代為引薦。\n";
        command("say 這位"+ RANK_D->query_respect(me)
               + "來得正好，這裡正有馬車要回庄辦事，就讓他們送你一程吧。\n");
...
int do_decide()
{
        ...
        message_vision("$N說道：這位"+RANK_D->query_respect(me) + "請先到敝庄盤恆數日，以後再圖相敘。\n", this_object());
        return 1;
}
