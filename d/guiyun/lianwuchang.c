...
void create()
{
        set("short","練武場");
        set("long",@LONG
這是供歸雲庄內弟子們練功的場子。場邊擺了兩排兵器架，上面插
着各種兵器，最多的還是各式各樣的劍。在場子的一個角落裡並排豎著
...
int do_jump(string arg)
{
        ...
        if (arg == "down")
        {
                if( !query_temp("thd_zhuang", me) )
                {
                        message_vision("$N想挖個坑跳進去，四處比划了一番，卻發現周圍的人都用異樣的眼光看着自己。\n", me);
                        return 1;
                }
...
int do_squat()
{
        ...
        if( !query_temp("thd_zhuang", me) )
        {
                message_vision("$N剛往地上一蹲，就有個庄丁飛跑過來，大聲喝道：“不準隨地大小便！”\n", me);
                return 1;
        }
        if( query("qi", me) <= qicost )
        {
...
int valid_leave(object me, string dir)
{
        ...
        if( dir == "west" && query("gender", me) != "男性" )
                return notify_fail("你打開門正要往裡走，忽然發現裡面都是光膀子的男弟子，你趕忙捂着臉退了出來。\n");
