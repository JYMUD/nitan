...
void create()
{
        set("short", "飯廳");
        set("long", @LONG
這是歸雲庄弟子們每日吃飯的地方，飯菜的香味充溢着整個房間。
屋裡擺着一張大桌子(table)和一些椅子(chair)。屋中站着幾個僕役。
LONG
        );
        set("exits", ([
...
int do_sit(string arg)
{
        ...
        if ( !arg || (arg != "chair" && arg != "yizi") )
                return notify_fail("你要坐在地板上？有個性！可惜的是這會影響歸雲庄的形象。\n");        
        ...
        if( query_temp("marks/food_served", me)){
                ...
                message_vision("$N發覺眾人的眼光都在盯着自己，忙站起身來，尷尬地說：我只是試試這椅子結實不結實。\n", me);
                return notify_fail("");
        }
