                ...
                if (userp(ob[i]))
                {
                        if( pay=query_temp("total_pay", ob[i]) )
                        {
                                ...
                                if( !query_temp("to_pay", ob[i]) )
                                {
                                        ...
                                        message_vision("掌柜翻出菜單，撥打了一陣算盤後告訴$N：“一共" + chinese_number(pay) + "文錢。”\n", ob[i]);
...
int do_order(string arg)
{
        ...
        if (me->is_busy() || me->is_fighting())
                return notify_fail("你正忙着呢！\n");
        if( !query_temp("marks/sit", me) )
                return notify_fail("酒保用好象瞧見一個怪物一樣的眼神打量着你：“站着吃喝？有位子不坐，您不嫌累呀？”\n");
        if (!arg)
...
int do_eat(string arg)
{
        ...
        if( !query_temp("marks/sit", me) )
        {
                write( "酒保用好象瞧見一個怪物一樣的眼神打量着你：“站着吃喝？有位子不坐，您不嫌累呀？”\n");
                return 1;
        }
        if (me->is_busy()) {
                write("你上一個動作還沒有完成。\n");
...
int do_pay()
{
        ...
        if( !(pay=query_temp("total_pay", me)) )
                return notify_fail("掌柜賠笑道：“您是來做善事是嗎？不吃也付帳？”\n");
        if( query_temp("marks/sit", me) )
        {
                message_vision("$N站起身來，走到櫃檯前結帳。\n", me);
                message_vision("掌柜翻出菜單，撥打了一陣算盤後告訴$N：“一共" + MONEY_D->price_str(pay) + "。”\n", me);
                message_vision("掌柜衝著酒保嚷道：把這位客官桌子上的剩菜收拾乾淨。\n", me);
        }
        else
        {
                message_vision("$N向掌柜問道：“還差多少？”\n", me);
                message_vision("掌柜翻了翻白眼，說道：“還差" + MONEY_D->price_str(pay) + "。”\n", me);
        }
        delete_temp("marks/sit", me);
