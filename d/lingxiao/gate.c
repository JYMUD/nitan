...
頂，卻有着一個如此去處。內門前那張厚厚的弔橋(bridge)正緊
...
void close_bridge()
{
        ...
        {
                delete("exits/south");
                message("vision", HIY "幾位凌霄弟子上前把弔橋關了起來。\n"
                                  NOR, this_object());
                delete("exits/north", room);
                message("vision", HIY "只聽“嘎嘎嘎嘎”幾聲，弔橋又被關了"
                                  "起來。\n" NOR, room);
        }
}
...
int do_open(string arg)
{
        ...
        if (objectp(room))
        {
                ...
                message_vision(HIY "$N" HIY "讓凌霄弟子把弔橋放了下來。\n" NOR, this_player());
                set("exits/north", __FILE__, room);
                message("vision", HIY "只聽“嘎嘎嘎嘎”幾聲，弔橋被放了下來。\n" NOR, room);
                remove_call_out("close_bridge");
