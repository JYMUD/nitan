...
"門弟子可以自由的進出，睡覺也是免費的。唐小二里里外外忙得團團轉，\n"
...
{          
        ...
        if( !query_temp("rent_paid", me) && dir == "up" )
                return notify_fail("唐小二一下擋在樓梯前，白眼一翻：怎麽着，想白住啊！這裡可是"+HIC"蜀中唐門"NOR+"啊\n");

        if( query_temp("rent_paid", me) && dir == "west" )
        {     
                myfam=query("family", me);
                if (myfam["family_name"] != "唐門世家")
                        return notify_fail("唐小二跑到門邊攔住：客官已經付了銀子，怎麽不住店就走了呢！旁人還以為小店伺候不周呢！\n");
